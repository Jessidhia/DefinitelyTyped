...
let hitOptionsInterfacePartial: paper.IHitTestOptions = {
    match: (hit: paper.HitResult) => {
        return true;
    }
};
let hitOptionsInterfaceFull: paper.IHitTestOptions = {
    tolerance: 0,
    class: "Path",
    match: (hit: paper.HitResult) => {
        return true;
...
hitTestResults = compoundPath.hitTestAll(
    hitTestPoint,
    hitOptionsInterfaceEmpty
);
hitTestResults = compoundPath.hitTestAll(
    hitTestPoint,
    hitOptionsInterfacePartial
);
...
hitTestResults = paper.project.hitTestAll(
    hitTestPoint,
    hitOptionsInterfaceEmpty
);
hitTestResults = paper.project.hitTestAll(
...
function Examples() {
    function BooleanOperations() {
        ...
        function onFrame(event: paper.IFrameEvent) {
            if (activeItem != ring) {
                ...
                let offset = new paper.Point(140, 80).multiply(
                    new paper.Point([
                        Math.sin(event.count / 60),
                        Math.sin(event.count / 40)
                    ])
                );
            ...
            function performOperation(
                operation: string,
                primaryItem: paper.PathItem,
                secondaryItem: paper.PathItem
            ) {
                switch (operation) {
                    case "unite":
    ...
    function CandyCrash() {
        ...
        for (let i = 0; i < numBalls; i++) {
            let position = paper.Point.random().multiply(
                new paper.Point(paper.view.size)
            );
    ...
    function SatieLikedToDraw() {
        ...
        paper.view.onFrame = function() {
            ...
            for (let i = 1; i <= amount; i++) {
                leftPath.segments[i].point = new paper.Point([
                    i * step,
                    -leftBands[i - 1] * scale
                ]);
                rightPath.segments[i].point = new paper.Point([
                    i * step,
                    -rightBands[i - 1] * scale * (flip ? -1 : 1)
                ]);
            }
            leftPath.smooth();
            rightPath.smooth();
    ...
    function TadPoles() {
        ...
        class Boid {
            ...
            calculateTail() {
                ...
                for (let i = 1; i < this.amount; i++) {
                    ...
                    point = point.add(
                        lastVector.normalize(pieceLength).add(sway)
                    );
        ...
        function onFrame(event: paper.IFrameEvent) {
            for (let i = 0, l = boids.length; i < l; i++) {
                if (groupTogether) {
                    let length =
                        (((i + event.count / 30) % l) / l) * heartPath.length;
    ...
    function NyanRainbow() {
        ...
        let mousePos = paper.view.center.add(
            new paper.Point([paper.view.bounds.width / 3, 100])
        );
        ...
        function onKeyDown(event: paper.KeyEvent) {
            if (event.key == "space")
                paper.project.activeLayer.selected = !paper.project.activeLayer
                    .selected;
        }

        let moveStars: (
            vector: paper.Point
        ) => void = moveStarsFunctionCreator();
        function moveStarsFunctionCreator(): (vector: paper.Point) => void {
            ...
            for (let i = 0; i < count; i++) {
                ...
                let center = paper.Point.random().multiply(
                    new paper.Point(paper.view.size)
                );
            ...
            return function(vector: paper.Point) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    item.position +=
                        vector.normalize(length) + item.data.vector;
                    keepInView(item);
                }
            };
        }
        ...
        let moveRainbow: (
            vector: paper.Point,
            event: paper.IFrameEvent
        ) => void = moveRainbowunctionCreator();
        function moveRainbowunctionCreator(): (
            vector: paper.Point,
            event: paper.IFrameEvent
        ) => void {
            ...
            function createHead(vector: paper.Point, count: number) {
                ...
                innerCircle.fillColor =
                    count % blinkTime < 3 || count % (blinkTime + 5) < 3
                        ? "black"
                        : "white";
            ...
            return function(v: paper.Point, event: paper.IFrameEvent) {
                let vector: paper.Point = paper.view.center
                    .subtract(position)
                    .divide(10);

                if (vector.length < 5) vector.length = 5;
                ...
                for (let j = 0; j < paths.length; j++) {
                    let path = paths[j];
                    let nyanSwing = playing
                        ? Math.sin(event.count / 2) * vector.length
                        : 1;
                    let unitLength =
                        (vector.length * (2 + Math.sin(event.count / 10))) / 2;
                    let length = (j - middle) * unitLength + nyanSwing;
                    let top = paper.view.center.add(rotated.normalize(length));
                    let bottom = paper.view.center.add(
                        rotated.normalize(length + unitLength)
                    );
                    path.add(top);
                ...
                if (mediaElement) mediaElement.setVolume(vector.length / 200);
            };
        }
    }
    ...
    function RadialRainbows() {
        ...
        let radius =
            Math.max(paper.view.size.width, paper.view.size.height) * 0.75;
    ...
    function MetaBalls() {
        ...
        function metaball(
            ...
            if (d > maxDistance || d <= Math.abs(radius1 - radius2)) {
                ...
                u1 = Math.acos(
                    (radius1 * radius1 + d * d - radius2 * radius2) /
                        (2 * radius1 * d)
                );
                u2 = Math.acos(
                    (radius2 * radius2 + d * d - radius1 * radius1) /
                        (2 * radius2 * d)
                );
            } else {
                u1 = 0;
    ...
    function Voronoi() {
        ...
        function generateBeeHivePoints(size: paper.Size, loose: boolean) {
            ...
            for (let i = -1; i < size.width + 1; i++) {
                for (let j = -1; j < size.height + 1; j++) {
                    let point = new paper.Point(i, j)
                        .divide(new paper.Point(size))
                        .multiply(new paper.Point(paper.view.size))
                        .add(new paper.Point(col).divide(2));
                    if (j % 2)
                    ...
                    if (loose)
                        point = point.add(
                            new paper.Point(col)
                                ...
                                .subtract(new paper.Point(col.divide(4)))
                        );
                    points.push(point);
                }
            }
        ...
        function onResize() {
            ...
            for (let i = 0, l = sites.length; i < l; i++) {
                sites[i] = sites[i].multiply(
                    new paper.Point(paper.view.size.divide(oldSize))
                );
            }
    ...
    function FutureSplash() {
        ...
        class Spring {
            ...
            update() {
                ...
                let normDistStrength =
                    ((dist - this.restLength) / (dist * this.mamb)) *
                    this.strength;
        ...
        function createPath(strength: number) {
            ...
            for (let i = 0; i <= values.amount; i++) {
                let segment = path.add(
                    new paper.Point(i / values.amount, 0.5).multiply(
                ...
                if (i > 0) {
                    let spring = new Spring(
                        segment.previous.point as SpringPoint,
                        point,
                        strength
                    );
                    springs.push(spring);
        ...
        function onMouseMove(event: paper.ToolEvent) {
            ...
            if (!point.fixed && location.distance < size.height / 4) {
                ...
                if (
                    segment.previous &&
                    !(segment.previous.point as SpringPoint).fixed
                ) {
                ...
                if (
                    segment.next &&
                    !(segment.next.point as SpringPoint).fixed
                ) {
        ...
        function onKeyDown(event: paper.KeyEvent) {
            if (event.key == "space") {
                ...
                path.fillColor = path.fullySelected
                    ? new paper.Color("")
                    : "black";
            }
        }
    ...
    function SpiralRaster() {
        ...
        function onResize() {
            ...
            text.point = paper.view.bounds.bottomRight.subtract(
        ...
        function onDocumentDrop(event: Event) {
            ...
            reader.onload = function(event) {
                ...
                if (event.target) image.src = (event.target as any).result;
            };
            ...
            raster.on("load", function() {
                ...
                for (let y = 0; y < values.amount; y++) {
                    for (let x = 0; x < values.amount; x++) {
                        ...
                        copy.position = copy.position.add(
                            new paper.Point(
                                size.multiply(
                                    new paper.Size([
                                        x + (y % 2 ? 0.5 : 0),
    ...
    function PathIntersections() {
        ...
        let words = paper.project.importSVG((document.getElementById(
            "svg"
        ...
        function onMouseMove(event: paper.MouseEvent) {
            ...
            for (let i = 0; i < yesGroup.children.length; i++) {
                for (let j = 0; j < noGroup.children.length; j++) {
                    showIntersections(
                        noGroup.children[j] as paper.PathItem,
                        yesGroup.children[i] as paper.PathItem
                    );
                }
            }
        }
    ...
    function HitTesting() {
        ...
        function createPaths() {
            ...
            for (let i = 0; i < values.paths; i++) {
                ...
                let points =
                    values.minPoints + Math.floor(Math.random() * pointsDelta);
                let path = createBlob(
                    paper.Point.random().multiply(
                        new paper.Point(paper.view.size)
                    ),
                    radius,
                    points
    ...
    function BouncingBalls() {
        class Ball {
            ...
            constructor(point: paper.Point, vector: paper.Point) {
                ...
                let gradient = new paper.Gradient(
                    [
                        new paper.GradientStop(color),
                        new paper.GradientStop(new paper.Color("black"))
                    ],
                    true
                );
                ...
                let ball = new paper.CompoundPath({
                    ...
                    fillColor: new paper.Color(
                        gradient,
                        new paper.Point([0]),
                        new paper.Point([radius]),
                        new paper.Point([radius / 8])
                    )
                });
            ...
            iterate() {
                ...
                if (pre.y < this.radius || pre.y > size.height - this.radius) {
                    if (Math.abs(this.vector.x) < 3)
                        this.vector = paper.Point.random().multiply(
                            new paper.Point([150, 100]).add(
                                new paper.Point([-75, 20])
                            )
                        );
                    this.vector.y *= this.bounce;
                }

                let max = paper.Point.max(
                    new paper.Point([this.radius]),
                    this.point.add(this.vector)
                );
                this.item.position = this.point = paper.Point.min(
                    max,
                    new paper.Point(size).subtract(this.radius)
                );
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let vector = paper.Point.random()
                .subtract(new paper.Point([0.5, 0]))
                .multiply(new paper.Point([50, 100]));
        ...
        function onFrame() {
            for (let i = 0, l = balls.length; i < l; i++) balls[i].iterate();
        }
    }
}
...
function APIReferenceExamples() {
    ...
    function CompoundPath1() {
        let pathData =
            "M20,50c0,-16.56854 13.43146,-30 30,-30c16.56854,0 30,13.43146 30,30c0,16.56854 -13.43146,30 -30,30c-16.56854,0 -30,-13.43146 -30,-30z M50,60c5.52285,0 10,-4.47715 10,-10c0,-5.52285 -4.47715,-10 -10,-10c-5.52285,0 -10,4.47715 -10,10c0,5.52285 4.47715,10 10,10z";
    ...
    function CompoundPath24() {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            // in the paper.view:
            let path = new paper.Path.Circle({
                center: paper.Point.random().multiply(
                    new paper.Point(paper.view.size)
                ),
                radius: 25,
                fillColor: "black",
    ...
    function Path2() {
        let pathData =
            "M100,50c0,27.614-22.386,50-50,50S0,77.614,0,50S22.386,0,50,0S100,22.386,100,50";
    ...
    function Path28() {
        ...
        // and {x: 170, y: 75}:
        path.addSegments([
            new paper.Point({ x: 100, y: 20 }),
            new paper.Point({ x: 170, y: 75 })
        ]);
    }
    function Path29() {
        let path = new paper.Path({
    ...
    function Path37() {
        ...
        let pointOnCircle = paper.view.center.add(
            new paper.Point({
                length: 40,
                angle: 30
            })
    ...
    function PathItem10() {
        ...
        // position of the mouse:
        function onMouseDrag(event: paper.ToolEvent) {
            if (myPath) myPath.arcTo(event.point);
        }
    }
    function PathItem11() {
        let myPath: paper.Path | null;
    ...
    function PointText56() {
        ...
        text.content =
            "Move your mouse over the paper.view, to see its position";
    ...
    function Style5() {
        let line = new paper.Path([
            new paper.Point(80, 50),
            new paper.Point(420, 50)
        ]);
    ...
    function SymbolDefinition0() {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            instance.position = paper.Point.random().multiply(
                new paper.Point(paper.view.size)
            );
    ...
    function Tool0() {
        ...
        paper.tool.onMouseDrag = function(
            this: paper.Tool,
            event: paper.ToolEvent
        ) {
