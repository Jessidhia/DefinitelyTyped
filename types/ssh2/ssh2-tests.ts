...
var pubKey = utils.genPublicKey(utils.parseKey(
    fs.readFileSync("user.pub")
) as ssh2_streams.ParsedKey);
            ...
            .on("authentication", ctx => {
                if (
                    ...
                    if (ctx.signature) {
                        ...
                        if (
                            verifier.verify(
                                pubKey.publicOrig.toString("utf8"),
                                ctx.signature
                            )
                        )
                            ctx.accept();
                        ...
                        sftpStream
                            ...
                            .on(
                                ...
                                (
                                    ...
                                    var inspected = require("util").inspect(
                                        data
                                    );
                                    console.log(
                            ...
                            .on("CLOSE", (reqid: any, handle: any) => {
                                ...
                                if (
                                    handle.length !== 4 ||
                                    !openFiles.has(
                                        (fnum = handle.readUInt32BE(0, true))
                                    )
                                )
                                    return sftpStream.status(
                                        reqid,
