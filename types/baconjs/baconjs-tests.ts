...
function CommonMethodsInEventStreamsAndProperties() {
...

    {
        ...
        Bacon.sequentially<string, { id: number; type: string; val?: number }>(
            2,
            events
        )
            .groupBy(keyF, limitF)
            .flatMap(groupedStream =>
                groupedStream.fold(0, (acc, x) => acc + x)
            )
            .onValue((sum: number) => {
...
function Errors() {
    ...
    Bacon.fromArray<string, number>([1, 2, 3, 4]).flatMapError<number>(
        error => {
...
function JoinPatterns() {
    {
        ...
        Bacon.when(
            [tick, keyEvent],
            (_: number, timestamp: number) => handleKeyEvent(timestamp),
            [tick],
            handleTick
        );
    ...
    }
    {
        ...
        Bacon.update(
            initial,
            [x, y, z],
            (previous: number, x: number, y: number, z: number) =>
                previous + x + y + z,
            [x, y],
    ...
    }
    {
        ...
        var scoreMultiplier = Bacon.constant(1),
            ...
            score = Bacon.update(
                ...
                (score: number, _: number, multiplier: number) =>
    ...
    }
    {
        ...
        let oxygen = Bacon.interval(1e3, "O"),
            ...
            makeCarbonMonoxide = (oxygen: string, carbon: string) =>
                `${carbon}${oxygen}`;
