...
var connection: sql.ConnectionPool = new sql.ConnectionPool(config, function(
    ...
    if (err != null) {
        ...
        requestStoredProcedure.execute("StoredProcedureName", function(
            err,
            recordsets,
            returnValue
        ) {
        ...
        requestStoredProcedure.execute<Entity>("StoredProcedureName", function(
            err,
            recordsets,
            returnValue
        ) {
...
function test_table2() {
    ...
    ([
        {
            name: "name",
            type: { typeName: "VarChar", length: sql.MAX },
            nullable: false
        },
        { name: "type", type: { typeName: "Int" }, nullable: false },
        {
            name: "type",
            type: { typeName: "Decimal", precision: 7, scale: 2 },
            nullable: false
        }
    ] as any[]).forEach((col: sql.IColumn) =>
        table.columns.add(col.name, _getSqlType(col.type), {
            nullable: col.nullable
        })
    );

    [["name", 42, 3.5], ["name2", 7, 3.14]].forEach((row: sql.IRow) =>
        table.rows.add(...row)
    );
}
