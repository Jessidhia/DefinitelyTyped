function test_application() {
    ...
    // removeEventListener
    const previousCallback = function(event: fin.WindowEvent) {};
...
function test_external_application() {
    ...
    // removeEventListener
    const previousCallback = function() {};
...
function test_inter_application_bus() {
    ...
    const aRegisteredListener = function(
        uuid: string,
        topic: string,
        name: string
    ) {};
    ...
    // subscribe
    fin.desktop.InterApplicationBus.subscribe("*", "a topic", function(
        message,
        uuid,
        name
    ) {
    ...
    // unsubscribe
    const aRegisteredMessageListener = function(
        message: any,
        senderUuid: string
    ) {
...
function test_system() {
    ...
    fin.desktop.System.getAllExternalApplications(externalAppsInfoList => {
        externalAppsInfoList.forEach(appInfo => {
            console.log(
                `External app connected to the runtime with UUID ${
                    appInfo.uuid
                }`
            );
        });
...
function test_system_clipboard() {
    ...
    fin.desktop.System.Clipboard.availableFormats(
        ...
        (reason, err) => {
            console.log(
                `Error while reading the clipboard Message: ${
                    err.message
                }, Stack: ${err.stack}`
            );
        }
