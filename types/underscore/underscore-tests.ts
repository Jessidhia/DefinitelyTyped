...
sum = _.reduce<string, number>(
    { a: "1", b: "2", c: "3" },
    (memo, numstr) => memo + +numstr
);
...
namespace TestFind {
...

    {
        ...
        result = _(array)
            .chain()
            .find<{ a: string }>(iterator)
            .value();
        result = _(array)
            .chain()
            .find<{ a: string }>(iterator, context)
            .value();
        result = _(array)
            .chain()
            .find<{ a: string }, { a: string }>({ a: "b" })
            .value();
        result = _(array)
            .chain()
            .find<{ a: string }>("a")
        ...
        result = _(array)
            .chain()
            .detect<{ a: string }>(iterator)
            .value();
        result = _(array)
            .chain()
            .detect<{ a: string }>(iterator, context)
            .value();
        result = _(array)
            .chain()
            .detect<{ a: string }, { a: string }>({ a: "b" })
            .value();
        result = _(array)
            .chain()
            .detect<{ a: string }>("a")
    ...
    {
        let iterator = (value: string, index: number, list: _.List<string>) =>
            value === "b";
        ...
        result = _("abc")
            .chain()
            .find<string>(iterator)
            .value();
        result = _("abc")
            .chain()
            .find<string>(iterator, context)
            .value();
        ...
        result = _("abc")
            .chain()
            .detect<string>(iterator)
            .value();
        result = _("abc")
            .chain()
            .detect<string>(iterator, context)
            .value();
    }
}
...
_.partition<number>([0, 1, 2, 3, 4, 5], num => {
    return num % 2 == 0;
});
...
_.filter(
    [{ name: "larry", relation: "father" }, { name: "moe", relation: "uncle" }],
    isUncleMoe
);
...
var updatePosition = (param: string) =>
    alert("updating position... Param: " + param);
var throttled = _.throttle(updatePosition, 100);
$(window).scroll(throttled);
throttled.cancel();

var calculateLayout = (param: string) =>
    alert("calculating layout... Param: " + param);
var lazyLayout = _.debounce(calculateLayout, 300);
$(window).resize(lazyLayout);
lazyLayout.cancel();

var createApplication = (param: string) =>
    alert("creating application... Param: " + param);
...
var partialApplicationTestFunction = (
    a: string,
    b: number,
    c: boolean,
    d: string,
...
_.mapObject({ a: 1, b: 2 }, val => val * 2) ===
    _.mapObject({ a: 2, b: 4 }, _.identity);
_.mapObject({ a: 1, b: 2 }, (val, key, o) => o[key] * 2) ===
    _.mapObject({ a: 2, b: 4 }, _.identity);
_.mapObject({ x: "string 1", y: "string 2" }, "length") ===
    _.mapObject({ x: "string 1", y: "string 2" }, _.property("length"));
...
_.template("Using 'with': <%= data.answer %>", oldTemplateSettings)({
    variable: "data"
});
...
function chain_tests() {
    ...
    var hoverOverValueShouldBeNumberNotAny = _([1, 2, 3])
        .chain()
        .map(num => [num, num + 1])
        .flatten()
        .find(num => num % 2 == 0)
