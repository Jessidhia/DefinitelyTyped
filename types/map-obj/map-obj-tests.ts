...
const obj = mapObj({ foo: "bar" }, (key, value) => [value, key], {
    target: { baz: "baz" }
});
...
const obj1 = mapObj({ foo: "bar" }, (key, value) => [value, key], {
    target: { baz: "baz" },
    deep: false
});
...
const obj2 = mapObj({ foo: "bar" }, (key, value) => [value, key], {
    deep: true
});
obj2; // $ExpectType object
const obj3 = mapObj({ foo: "bar" }, (key, value) => [value, key], {
    deep: true,
    target: { bar: "baz" }
});
