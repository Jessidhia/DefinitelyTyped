...
class SortableComponent extends React.Component<{}, SortableComponentState> {
    ...
    private _handleSortEnd(
        sort: ReactSortableHOC.SortEnd,
        event: ReactSortableHOC.SortEvent
    ): void {
        this.setState({
            items: ReactSortableHOC.arrayMove(
                this.state.items,
                sort.oldIndex,
                sort.newIndex
            )
    ...
    private _getHelperDimensions(
        sort: ReactSortableHOC.SortStart
    ): ReactSortableHOC.Dimensions {
