...
function stringify(cmd: svgParser.Command) {
    if (isMoveToCommand(cmd)) {
        ...
        return `${cmd.code} ${cmd.x1} ${cmd.y1} ${cmd.x2} ${cmd.y2} ${cmd.x} ${
            cmd.y
        }`;
        ...
        return `${cmd.code} ${cmd.rx} ${cmd.ry} ${cmd.xAxisRotation} ${
            cmd.largeArc
        } ${cmd.sweep} ${cmd.x} ${cmd.y}`;
    } else {
        throw new Error(
            `${cmd.code} command is unknown command: cmd = ${JSON.stringify(
                cmd
            )}`
        );
    }
