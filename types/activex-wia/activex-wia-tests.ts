                ...
                if (p.IsReadOnly) {
                    ...
                    switch (p.SubType) {
                        ...
                        case WIA.WiaSubType.RangeSubType:
                            s += ` [valid values in the range from ${
                                p.SubTypeMin
                            } to ${p.SubTypeMax} in increments of ${
                                p.SubTypeStep
                            }]`;
                            break;
                    }
...
Frame count = ${img.FrameCount}}
        ...
        const listProperties = (filter: WIA.Filter) => {
            ...
            s += collectionToArray(filter.Properties)
                .map(p => {
                        ...
                        default:
                            switch (p.SubType) {
                                ...
                                case WIA.WiaSubType.ListSubType:
                                    ...
                                    break;
                                case WIA.WiaSubType.RangeSubType:
                                    contents = ` // [valid values between ${
                                        p.SubTypeMin
                                    } and ${p.SubTypeMax}, with a step of ${
                                        p.SubTypeStep
                                    }]`;
                                    break;
                                default:
                    ...
                    return `ip.Filters(1).Properties("${
                        p.Name
                    }") = ${contents}`;
                })
                .join("\n");
        ...
        if (dev) {
            for (const item of collectionToArray(dev.Items)) {
                ...
                if (item.Properties.Exists("Item Time Stamp")) {
                    const v: WIA.Vector = item.Properties("Item Time Stamp")
                        .Value;
                    if (v.Count === 8) {
                        s += ` (${v.Date})`;
                    }
    ...
    // Implement a windows script host script that runs automatically
    {
        ...
        switch (args.length) {
            case 1:
            case 2:
                const command = `${WScript.FullName} "${
