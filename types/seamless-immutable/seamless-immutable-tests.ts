...
// ---------------------------------------------------------------
{
    ...
    const filter: Immutable.Immutable<User[]> = array.filter(
        (value: User) => value.firstName === "test"
    );
    filter.asMutable();
    ...
    const concat: Immutable.Immutable<User[]> = array.concat({
        firstName: "Happy",
        lastName: "Cat"
    });
    concat.asMutable();
    ...
    const reduce1: Immutable.Immutable<User> = array.reduce(
        (previous, current) => ({ ...previous, lastName: current.lastName })
    );
    reduce1.asMutable();
    ...
    const arrayToObject1: Immutable.Immutable<
        ArrayToObjectResult
...

{
    ...
    const updatedUser11: Immutable.Immutable<
        ExtendedUser
    > = immutableUserEx.setIn(["address", "line1"], "Small house");
    const updatedUser12: Immutable.Immutable<
        ExtendedUser
    ...
    const updatedUser42: Immutable.Immutable<User> = immutableUser.update<
        string
    >(data.propertyId, x => x.toLowerCase() + " Whirlwind");
    ...
    // the type of the updated value must be explicity specified in case of fallback
    const updatedUser52: Immutable.Immutable<User> = immutableUserEx.updateIn<
        string
    >([data.propertyId, "line1"], x => x.toLowerCase() + " 43");

    // without
    const simpleUser1: Immutable.Immutable<User> = immutableUserEx.without(
        "address"
    );
