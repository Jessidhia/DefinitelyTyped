...
namespace MeteorTests {
    ...
    console.log(
        error.reason === "The user must be logged in to post a comment."
    );
    ...
    var myMessages: any[] = Messages.find({
 ...
 **/
    ...
    var Animals = new Mongo.Collection<AnimalDAO>("Animals", {
        transform: function(doc: any): Animal {
    ...
    var handle = query.observeChanges({
        added: function(id: string, user: { name: string }) {
            count1++;
            console.log(
                user.name + " brings the total to " + count1 + " admins."
            );
        },
        removed: function() {
            count1--;
    ...
    Comments.find({
        $or: [{ text: "hello" }, { text: "world", viewNumber: { $gt: 0 } }],
    ...
    Comments.find({
        $query: { inlineLinks: { $exists: true, $type: "array" } }
    });
    Comments.find({
        inlineLinks: {
            $elemMatch: {
    ...
    Comments.update(
        { viewNumber: { $exists: false } },
        { $set: { viewNumber: 0 } }
    );
    Comments.update(
        { authorId: "author-id-1" },
        { $push: { tags: "test-tag-1" } }
    );
    Comments.update(
        { authorId: "author-id-1" },
        { $push: { tags: { $each: ["test-tag-2", "test-tag-3"] } } }
    );
    ...
    Accounts.onCreateUser(function(
        options: { profile: any },
        user: { profile: any; dexterity: number }
    ) {
        var d6 = function() {
    ...
    Meteor.methods({
        addChat: function(
            roomId: string,
            message: { text: string; timestamp: Date; tags: string }
        ) {
            check(roomId, String);
            check(message, {
                text: String,
    ...
    var reactiveVar2 = new ReactiveVar<string>("test value", function(
        oldVal: any
    ) {
        return true;
    });
    ...
    Meteor.loginWithMeteorDeveloperAccount(loginOpts, function(
        error: Meteor.Error
    ) {});
    ...
    var handle = Accounts.validateLoginAttempt(function(
        attemptInfoObject: Accounts.IValidateLoginAttemptCbOpts
    ) {
    ...
    const deeperPublicSetting =
        Meteor.settings.public["somePublicSetting"]["deeperSetting"];
    const privateSetting = Meteor.settings["somePrivateSetting"];
    const deeperPrivateSetting =
        Meteor.settings["somePrivateSettings"]["deeperSetting"];

    // Covers https://github.com/meteor-typings/meteor/issues/9
    const username = (Template.instance().find("#username") as HTMLInputElement)
        .value;
    ...
    const collectionWithoutConnection = new Mongo.Collection<MonkeyDAO>(
        "monkey",
        {
            connection: null
        }
