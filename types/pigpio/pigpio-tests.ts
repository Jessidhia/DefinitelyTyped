...
(function gpio_mode(): void {
    ...
    assert.strictEqual(
        gpio7.getMode(),
        Gpio.INPUT,
        "expected INPUT mode for gpio7"
    );
    assert.strictEqual(
        gpio8.getMode(),
        Gpio.OUTPUT,
        "expected OUTPUT mode for gpio8"
    );
    ...
    assert.strictEqual(
        gpio7.getMode(),
        Gpio.OUTPUT,
        "expected OUTPUT mode for gpio7"
    );
...
(function gpio_numbers(): void {
    ...
    assert.strictEqual(
        Gpio.MAX_USER_GPIO,
        31,
        "expected Gpio.MAX_USER_GPIO to be 31"
    );
})();
...
(function isr_enable_disable(): void {
    ...
    function noEdge() {
        ...
        setTimeout(function timer() {
            console.log(
                "    " + risingCount + " rising edge interrupts (0 expected)"
            );
            console.log(
                "    " + fallingCount + " falling edge interrupts (0 expected)"
            );
    ...
    function fallingEdge() {
        ...
        setTimeout(function timer() {
            ...
            console.log(
                "    " +
                    fallingCount +
                    " falling edge interrupts (~50 expected)"
            );
    ...
    function risingEdge() {
        ...
        setTimeout(function timer() {
            console.log(
                "    " + risingCount + " rising edge interrupts (~50 expected)"
            );
            console.log(
                "    " + fallingCount + " falling edge interrupts (0 expected)"
            );
...
(function isr_multiple_sources(): void {
    ...
    [[7, 8], [9, 11]].forEach(function loop(gpioNos: [number, number]) {
        ...
        const input = new Gpio(gpioNos[0], {
            mode: Gpio.INPUT,
...
(function isr_performance(): void {
    ...
    setTimeout(function timer() {
        ...
        setTimeout(function timer() {
            time = process.hrtime(time);
            const interruptsPerSec = Math.floor(
                interruptCount / (time[0] + time[1] / 1e9)
            );
...
(function isr_timeouts_2(): void {
    ...
    setTimeout(function time() {
        ...
        setTimeout(function timer() {
            input.disableInterrupt();
            console.log(
                "  " + timeoutCount + " timeouts detected (~1000 expected)"
            );
        }, 1000);
    }, 1000);
})();
...
(function isr_timeouts(): void {
    ...
    setTimeout(function timer() {
        ...
        setTimeout(function timer() {
            input.disableInterrupt();
            console.log(
                "  " + timeoutCount + " timeouts detected (0 expected)"
            );
        }, 1000);
    }, 1000);
})();
    ...
    (function closure() {
        ...
        ledNotifier.stream().on("data", function event(buf: Buffer) {
            for (
                let ix = 0;
                ix < buf.length;
                ix += Notifier.NOTIFICATION_LENGTH
            ) {
...
(function notifier_stress(): void {
    ...
    (function closure() {
        ...
        ledNotifier.stream().on("data", function event(buf: Buffer) {
            ...
            restBuf =
                rest === 0 ? null : new Buffer(buf.slice(buf.length - rest));
    ...
    (function closure() {
        ...
        const iv = setInterval(function timer() {
            ...
            const diff = Math.floor(
                (time[0] * 1e9 + time[1] - (lastTime[0] * 1e9 + lastTime[1])) /
                    1000
            );
            ...
            if (ledToggles === LED_TOGGLES) {
                ...
                console.log(
                    "  min setInterval diff: %d us",
                    minSetIntervalDiff
                );
                console.log(
                    "  max setInterval diff: %d us",
                    maxSetIntervalDiff
                );
            }
        }, 1);
...
(function pwm(): void {
    ...
    assert.strictEqual(
        led.getPwmRealRange(),
        250,
        "expected pwm real range to be 250"
    );
    assert.strictEqual(
        led.getPwmFrequency(),
        800,
        "expected get pwm frequency to be 800"
    );
    ...
    assert.strictEqual(
        led.getPwmRealRange(),
        100,
        "expected pwm real range to be 100"
    );
    assert.strictEqual(
        led.getPwmFrequency(),
        2000,
        "expected get pwm frequency to be 2000"
    );
    ...
    assert.strictEqual(
        led.getPwmDutyCycle(),
        dutyCycle,
        "expected duty cycle to be " + dutyCycle
    );
    ...
    assert.strictEqual(
        led.getPwmRealRange(),
        25,
        "expected pwm real range to be 25"
    );
    assert.strictEqual(
        led.getPwmFrequency(),
        1e7,
        "expected get pwm frequency to be 1e7"
    );
    assert.strictEqual(
        led.getPwmDutyCycle(),
        500000,
        "expected duty cycle to be 500000"
    );
...
(function tickAndTickDiff(): void {
    ...
    setTimeout(() => {
        ...
        assert.ok(
            diff > 0,
            "expected tick count to increase across a timer call"
        );
    }, 50);
})();
...
(function gpio_glitch_filter(): void {
    ...
    setTimeout(() => {
        assert.strictEqual(
            count,
            1,
            "expected 1 alert function call instead of " + count
        );
        console.log("  success...");
        process.exit(0);
    }, 1000);
