...
const emoji_string: string = emoji.emojify(
    "I :heart:  :coffee:! -  :hushed::star::heart_eyes:  ::: test : : :+1:+"
);

const emoji_string2: string = emoji.emojify(
...
const replaced_emoji: string = emoji.replace(
    "⚠️ 〰️ 〰️ low disk space",
    emoji => `${emoji.key}:`
);

const replaced_emoji_with_string: string = emoji.replace(
    "⚠️ 〰️ 〰️ low disk space",
    "example"
);

const replaced_emoji_keep_spaces: string = emoji.replace(
    "⚠️ 〰️ 〰️ low disk space",
    "",
    false
);
