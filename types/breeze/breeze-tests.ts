...
function test_entityAspect() {
    ...
    var orderDateErrors = order.entityAspect.getValidationErrors(
        orderDateProperty
    );
    order.entityAspect
    ...
    order.entityAspect.validationErrorsChanged.subscribe(function(
        validationChangeArgs
    ) {
...
function test_entityKey() {
    ...
    var empTerrKey = new breeze.EntityKey(<breeze.EntityType>empTerrType, [
        1,
        77
    ]);
    var empType = em1.metadataStore.getEntityType("Employee");
...
function test_metadataStore() {
    ...
    window.localStorage.setItem("metadata", metadataAsString);
    var metadataFromStorage = window.localStorage.getItem("metadata");
    var newMetadataStore = breeze.MetadataStore.importMetadata(
        metadataFromStorage
    );
    var metadataAsString = ms.exportMetadata();
    window.localStorage.setItem("metadata", metadataAsString);
...
function test_entityQuery() {
    ...
    var query = breeze.EntityQuery.fromEntityNavigation(employee, <
        breeze.NavigationProperty
    >ordersNavProp);
...
function test_event() {
    ...
    core.Event.enable("validationErrorsChanged", myEntityManager, function(
    ...
    salaryEvent.publish({ eventType: "payRaise", amount: 100 }, true, function(
        error
    ) {});
    salaryEvent.publishAsync({ eventType: "payRaise", amount: 100 });
    salaryEvent.publishAsync({ eventType: "payRaise", amount: 100 }, function(
        error
    ) {});
    ...
    var token = order.entityAspect.propertyChanged.subscribe(function(
        pcEvent
    ) {});
