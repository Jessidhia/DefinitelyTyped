...
tape("binary", t => {
    ...
    function testBinary1D(P: ndarray, Q: ndarray, testName: string) {
        ...
        for (let i = 0; i < P.shape[0]; ++i) {
            if (!(P.get(i) === i + 1001)) {
                t.fail(
                    `${testName}; encountered ${P.get(i)} instead of ${i +
                        1001} at ${i}`
                );
                return;
            }
...
tape("offset", t => {
    ...
    function testBinary1D(P: ndarray, Q: ndarray, testName: string) {
        ...
        for (let i = 0; i < P.shape[0]; ++i) {
            if (!(P.get(i) === 2 * i + 1001)) {
                t.fail(
                    `${testName}; encountered ${P.get(i)} instead of ${2 * i +
                        1001} at ${i}`
                );
                return;
            }
...
tape("unary", t => {
    ...
    function testUnary1D(arr: ndarray, testName: string) {
        ...
        for (let i = 0; i < arr.shape[0]; ++i) {
            if (!(arr.get(i) === i + 1)) {
                t.fail(
                    `${testName}; encountered ${arr.get(i)} instead of ${i +
                        1} at ${i}`
                );
                return;
            }
