...
if (localStorage.getItem("lastCartId")) {
    client.checkout
        .fetch(localStorage.getItem("lastCartId"))
        .then(function(remoteCart: ShopifyBuy.Cart) {
            cart = remoteCart;
...
client.product
    .fetch("3614436099")
    .then(function(fetchedProduct: ShopifyBuy.Product) {
        product = fetchedProduct;
        var selectedVariant: ShopifyBuy.ProductVariant =
...
function bindEventListeners() {
    ...
    $(document).on("click", function(evt) {
        if (
            !$(evt.target).closest(".cart").length &&
...
function updateQuantity(fn: Function, variantId: string | number) {
    var variant: ShopifyBuy.ProductVariant = product.variants.filter(function(
...
============================================================ */
function updateVariantInCart(
    cartLineItem: ShopifyBuy.LineItem,
    quantity: number
) {
...
function addVariantToCart(
    ...
    client.checkout
        ...
        .then(function() {
            var cartItem: ShopifyBuy.LineItem = cart.lineItems.filter(function(
