...
module DXDemo {
    ...
    export function onChangeStartFilterButtonClick(
        ...
        if (ASPxClientEdit.ValidateGroup("DateEditors")) {
            var params = getCurrentQueryParams();
            params["checkin"] = getFormattedDate(<Date>(
                checkInDateEdit.GetValue()
            ));
            params["checkout"] = getFormattedDate(<Date>(
                checkOutDateEdit.GetValue()
            ));
    ...
    function getParamsByQueryString(
        queryString: string
    ): { [key: string]: string } {
