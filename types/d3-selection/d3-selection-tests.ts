...
const topSelection: d3Selection.Selection<
    HTMLElement,
    any,
    null,
    undefined
...
let body: d3Selection.Selection<
    HTMLBodyElement,
    BodyDatum,
    HTMLElement,
    any
...
// $ExpectError
const body2: d3Selection.Selection<
    HTMLElement,
    any,
    null,
    undefined
...
const body3: d3Selection.Selection<
    HTMLBodyElement,
    any,
    null,
    undefined
...
const body7: d3Selection.Selection<
    ...
    HTMLElement,
    undefined
> = d3Selection.select<HTMLBodyElement | null, any>(maybeSVG1.node());
...
const xSVGCircleElementList: NodeListOf<
    SVGCircleElement
...
const svgEl: d3Selection.Selection<
    SVGSVGElement,
    SVGDatum,
    HTMLElement,
    any
...
let firstG: d3Selection.Selection<
    SVGGElement,
    SVGDatum,
    HTMLElement,
    any
...
divs = divs
    .style("padding", function(d, i, g) {
        ...
        const group:
            | HTMLDivElement[]
            | d3Selection.ArrayLike<HTMLDivElement> = g;
        console.log("Client Rectangle Top: ", this.getBoundingClientRect().top); // this context HTMLDivElement
        if (g.length > 0) {
            console.log("Alignment of first group element:", g[0].align); // group : Array<HTMLDivElement>
        }
...
body = body.html(function(d, i, g) {
    ...
    return `<div> Body Background Color: ${
        this.bgColor
    }, Foo Datum: ${d.foo}</div>`; // this context HTMLBodyElement, datum BodyDatum
});
...
function customListener(
    ...
    return `${
        e.team
    } defeated ${finalOpponent} in the EURO 2016 Cup. Who would have thought!!!`;
}
...
const svgTextObject:
    | d3Selection.NamespaceLocalObject
    | string = d3Selection.namespace("svg:text");
