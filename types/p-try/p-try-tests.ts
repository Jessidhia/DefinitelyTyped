...
pTry(b, "test", 1).then(v => {
    str = v;
});
pTry(c, "test", 1, false).then(v => {
    str = v;
});
pTry(d, "test", 1, false, Symbol("test")).then(v => {
    str = v;
});
pTry(e, "test", 1, false, Symbol("test"), "no").then(v => {
    str = v;
});
pTry(f, "test", 1, false, Symbol("test"), "no", 2).then(v => {
    str = v;
});
pTry(g, "test", 1, false, Symbol("test"), "no", 2, true).then(v => {
    str = v;
});
