...
stripe.balance
    .retrieveTransaction("txn_17xMvmBoqMA9o2xkYNH2ewNj")
...
stripe.charges
    ...
    .then(charge => {
        ...
        charge.refunds.retrieve("re_15jzA4Ee31JkLCeQcxbTbjaL").then(refund => {
            const status: "pending" | "succeeded" | "failed" | "canceled" =
                refund.status;
        });

        charge.refunds
            .update("re_15jzA4Ee31JkLCeQcxbTbjaL", {
                metadata: { test: "data" }
            })
            .then(refund => {});
...
stripe.charges
    .capture("ch_15fvyXEe31JkLCeQOo0SwFk9", { amount: 1 }, {})
    .then(charge => {
        // asynchronously called
    });
...
stripe.charges
    .listRefunds("ch_15fvyXEe31JkLCeQOo0SwFk9", { limit: 3 })
    .then(refunds => {
        // asynchronously called
    });
...
stripe.charges.markAsFraudulent(
    "ch_15fvyXEe31JkLCeQOo0SwFk9",
    (err, refunds) => {
        // asynchronously called
    }
...
stripe.customers
    ...
    .then(customer => {
        // asynchronously called
        customer.cards
            .create({ card: "tok_17wV94BoqMA9o2xkhlAd3ALf" })
            .then(customer => {});
        ...
        customer.cards
            .update("card_17xMvXBoqMA9o2xkq6W5gamx", { name: "Test" })
            .then(card => {});
        customer.cards.list().then(cards => {});
        customer.cards
            .del("card_17xMvXBoqMA9o2xkq6W5gamx")
            .then(confirmation => {});

        customer.subscriptions
            .create({ items: [{ plan: "gold" }], trial_period_days: 7 })
            .then(subscription => {});
        customer.subscriptions
            .create({
                items: [{ plan: "gold" }],
                trial_end: "now",
                billing_cycle_anchor: 1516881177,
                prorate: true
            })
            .then(subscription => {});
        customer.subscriptions
        ...
        customer.subscriptions
            ...
            .then(subscription => {});
        customer.subscriptions
            .retrieve("sub_8Eluur5KoIKxuy")
            .then(subscription => {
                customer.subscriptions
                    .update("sub_8Eluur5KoIKxuy", {
                        items: [
                            {
        ...
        customer.subscriptions
            .del("sub_8Eluur5KoIKxuy")
            .then(subscription => {});
        customer.subscriptions
            .deleteDiscount("sub_8Eluur5KoIKxuy")
            .then(confirmation => {});
...
stripe.customers
    .createCard("cus_5rfJKDJkuxzh5Q", { card: "tok_15V2YhEe31JkLCeQy9iUgsJX" })
    .then(card => {
        // asynchronously called
    });
...
stripe.customers
    .retrieveCard("cus_5rfJKDJkuxzh5Q", "card_15fvyXEe31JkLCeQ9KMktP5S")
    .then(card => {
        // asynchronously called
        const obj: Stripe.ICard = card;
    });
...
stripe.customers
    .updateCard("cus_5rfJKDJkuxzh5Q", "card_15fvyXEe31JkLCeQ9KMktP5S", {
        name: "Jane Austen"
    })
    .then(card => {
        // asynchronously called
...
stripe.customers
    .deleteCard("cus_5rfJKDJkuxzh5Q", "card_15fvyXEe31JkLCeQ9KMktP5S")
    .then(confirmation => {
        // asynchronously called
    });

stripe.customers.listCards(
    "cu_15fvyVEe31JkLCeQvr155iqc",
    null,
    (err, cards) => {
        // asynchronously called
    }
...
stripe.customers
    .listSources("cu_15fvyVEe31JkLCeQvr155iqc", null)
    .then(cards => {
        // asynchronously called
    });
stripe.customers
...
stripe.customers
    .retrieveSubscription("cus_5rfJKDJkuxzh5Q", "sub_5rfJxnBLGSwsYp")
    .then(subscription => {
        // asynchronously called
    });
...
stripe.customers
    .cancelSubscription("cus_5rfJKDJkuxzh5Q", "sub_5rfJxnBLGSwsYp", null)
    .then(confirmation => {
        // asynchronously called
    });
...
stripe.customers
    .cancelSubscription("cus_5rfJKDJkuxzh5Q", "sub_5rfJxnBLGSwsYp")
    .then(confirmation => {
        // asynchronously called
    });
...
stripe.customers
    .listSubscriptions("cu_15fvyVEe31JkLCeQvr155iqc", null)
    .then(subscriptions => {
        // asynchronously called
    });
...
stripe.customers
    .listSubscriptions("cu_15fvyVEe31JkLCeQvr155iqc")
    .then(subscriptions => {
        // asynchronously called
    });
...
stripe.customers.deleteSubscriptionDiscount(
    "cus_5rfJKDJkuxzh5Q",
    "sub_5rfJxnBLGSwsYp",
    (err, confirmation) => {
        // asynchronously called
...
stripe.customers
    .deleteSubscriptionDiscount("cus_5rfJKDJkuxzh5Q", "sub_5rfJxnBLGSwsYp")
    .then(confirmation => {
        // asynchronously called
    });
...
stripe.transfers
    .createReversal("tr_17F2JBFuhr4V1legrq97JrFE")
    .then(reversal => {
        // asynchronously called
    });
...
stripe.accounts
    .reject("acct_17wV8KBoqMA9o2xk", { reason: "fraud" })
    .then(account => {
        // asynchronously called
    });
...
stripe.accounts
    .createLoginLink("acct_17wV8KBoqMA9o2xk", "http://localhost:3000")
    .then(loginLink => {
        const object: string = loginLink.object;
        const created: number = loginLink.created;
        const url: string = loginLink.url;
...
stripe.accounts.createExternalAccount(
    "",
    { external_account: "btok_8E264Lxsbyvj3E" },
    (err, extAcc) => {
...
stripe.invoices
    .retrieveLines("in_15fvyXEe31JkLCeQH7QbgZZb", { limit: 5 })
    .then(lines => {
        // asynchronously called
    });
...
stripe.invoices
    .pay("in_15fvyXEe31JkLCeQH7QbgZZb", { source: "source_id" })
    .then(invoice => {
        // asynchronously called
    });
...
stripe.invoices
    .list({ customer: "cus_5rfJKDJkuxzh5Q", limit: 3 })
    .then(invoices => {
        // asynchronously called
    });

stripe.invoices
    .retrieve("in_15fvyXEe31JkLCeQH7QbgZZb", { expand: ["subscription"] })
    .then(invoice => {
...
stripe.payouts
    .retrieve("po_5rfJKDJkuxzh5Q", { stripe_account: "acct_abc12345678" })
    .then(payout => {});
...
stripe.payouts
    .update("po_5rfJKDJkuxzh5Q", { metadata: { key: "value" } })
    .then(payout => {});
stripe.payouts.update(
    "po_5rfJKDJkuxzh5Q",
    { metadata: { key: "value" } },
...
stripe.payouts
    .list({ limit: 100 }, { stripe_account: "acct_abc12345678" })
    .then(payouts => {});
...
stripe.payouts
    .cancel("po_5rfJKDJkuxzh5Q", { stripe_account: "acct_abc12345678" })
    .then(payout => {});
...
stripe.plans
    .update("gold-plan", { nickname: "New gold plan nickname" })
    .then(plan => {
        // asynchronously called
    });
...
stripe.plans.list(
    { active: true, product: "prod_someproduct" },
    (err, plans) => {
        // asynchronously called
    }
);
...
stripe.subscriptions.list(
    { customer: "cus_5rfJKDJkuxzh5Q", plan: "platypi-dev" },
    (err, subscriptions) => {
        // asynchronously called
    }
);
stripe.subscriptions
    .list({ customer: "cus_5rfJKDJkuxzh5Q", plan: "platypi-dev" })
    .then(subscriptions => {
        // asynchronously called
    });
...
stripe.subscriptionItems.retrieve(
    "si_C9gimdd2l9qvCU",
    (err, subscriptionItem) => {
        // asynchronously called
    }
...
stripe.subscriptionItems
    .update("si_C9gimdd2l9qvCU", { plan: "platypi" })
    .then(subscriptionItem => {
        // asynchronously called
    });
...
stripe.subscriptionItems.list(
    { subscription: "si_C9gimdd2l9qvCU" },
    (err, subscriptionItems) => {
        // asynchronously called
    }
);
stripe.subscriptionItems
    .list({ subscription: "si_C9gimdd2l9qvCU" })
    .then(subscriptionItems => {
        // asynchronously called
    });
...
stripe.usageRecords
    .create("sub_8QwCiwZ9tmMSpt", {
        action: "set",
        quantity: 10000,
        timestamp: 1537006853
    })
...
stripe.usageRecordSummaries
    .list({ subscription_item: "si_C9gimdd2l9qvCU", limit: 10 })
    .then(
        (
            usageRecordSummaries: Stripe.usageRecordSummaries.IUsageRecordSummaries
        ) => {}
    );
stripe.usageRecordSummaries.list(
    { subscription_item: "si_C9gimdd2l9qvCU", limit: 10 },
    (
        err,
        usageRecordSummaries: Stripe.usageRecordSummaries.IUsageRecordSummaries
    ) => {}
