...
async function main() {
    ...
    for (const interfaceGroup of interfaceGroups) {
        ...
        const conflicts = interfaceGroups.filter(
            g => g.interfaces[0].name === interfaceName
        );
        if (
            conflicts.length > 1 &&
            _.some(
                conflicts,
                c => !_.isEqual(c.interfaces, conflicts[0].interfaces)
            )
        ) {
            for (const conflict of conflicts) {
                ...
                for (const interfaceDef of conflict.interfaces) {
                    interfaceDef.name = interfaceDef.name.replace(
                        oldName,
                        newName
                    );
    ...
    const fpFile = [
        ...
        "    interface LoDashFp {",
        ...interfaceGroups.map(
            g => `        ${g.functionName}: ${g.interfaces[0].name};`
        ),
        "        __: lodash.__;",
...
async function processDefinitions(
    ...
    const builder: {
        [name: string]: (...args: number[][]) => () => Interface[];
    } = {};
    const unconvertedBuilder: {
        [name: string]: (...args: number[][]) => () => Interface[];
    } = {};
        ...
        for (const definition of definitions) {
            ...
            if (
                definition.overloads.every(
                    o =>
                        o.params.length <= 1 &&
                        (o.returnType === "typeof _" ||
                            o.returnType === "LoDashStatic")
                )
            ) {
                ...
                unconvertedBuilder[definition.name] = (...args: number[][]) => {
                    return () =>
                        curryDefinition(definition, args[0] || [], -1, false);
                };
            } else {
                builder[definition.name] = (
                    ...
                    if (spreadIndex !== -1) {
                        ...
                        const arity =
                            Math.max(spreadIndex, args[spreadIndex] as number) +
                            1;
                        args = args.slice(0, arity);
                    } else if (
                        args.length > 4 ||
                        definition.name === "flow" ||
                        definition.name === "flowRight"
                    ) {
    ...
    const interfaceGroups: InterfaceGroup[] = functionNames.map(
        (functionName): InterfaceGroup => ({
            ...
            interfaces: builderFp[functionName]([0], [1], [2], [3], [4])(
                [0],
                [1],
                [2],
                [3],
...
function parseDefinitions(
    ...
    if (!overloadMatch) {
        const lineNumber = getLineNumber(definitionString, startIndex);
        console.warn(
            `No function definitions were found in interface at ${filePath} line ${lineNumber}.`
        );
        return [];
    }
    const definitons: Definition[] = [];
    ...
    for (
        ;
        overloadMatch && overloadMatch.index < endIndex;
        overloadMatch = overloadRegExp.exec(definitionString)
    ) {
        ...
        if (!currentDefinition || name !== currentDefinition.name) {
            ...
            const jsdocStartIndex = definitionString.lastIndexOf(
                "/**",
                overloadStartIndex
            );
            const jsdocEndIndex = definitionString.indexOf(
                "*/",
                jsdocStartIndex
            );
            if (
                ...
                const overloadTestIndex = definitionString.indexOf(
                    "     " + name,
                    jsdocEndIndex
                );
        ...
        let overloadEndIndex = definitionString.indexOf(
            ";",
            overloadStartIndex
        );
        if (overloadEndIndex === -1 || overloadEndIndex > endIndex) {
            ...
            console.warn(
                `Overload does not end with semicolon! ${filePath} line ${lineNumber}`
            );
            break;
        }
        ...
        const overloadString = definitionString.substring(
            overloadStartIndex,
            overloadEndIndex
        );
        ...
        if (paramStartIndex !== -1) {
            ...
            if (
                typeParamStartIndex !== -1 &&
                typeParamStartIndex < paramStartIndex
            ) {
                const typeParamEndIndex = overloadString.indexOf(
                    ">(",
                    typeParamStartIndex
                );
            ...
            if (paramEndIndex === -1) {
                console.warn(
                    `Failed to find parameter end position in overload for '${name}' (${filePath}).`
                );
                continue;
            }
            overload.params = overloadString
                ...
                .map(o => _.trim(o, ","))
                .filter(o => !!o);
            overload.returnType = overloadString
                .substring(paramEndIndex + 2)
                .trim();
            ...
            if (
                interfaceName.startsWith("typeof ") ||
                interfaceName.startsWith("LoDashStatic[")
            ) {
            ...
            if (!interfaceName.startsWith("{")) {
                ...
                interfaceEndIndex = definitionString.indexOf(
                    `${lineBreak}${interfaceMatch[1]}}`,
                    interfaceStartIndex
                );
                ...
                interfaceStartIndex =
                    definitionString.indexOf(
                        "{",
                        overloadStartIndex + overloadParts[0].length
                    ) + 1;
                interfaceEndIndex = definitionString.indexOf(
                    "}",
                    interfaceStartIndex
                ...
                if (interfaceEndIndex === -1 || interfaceEndIndex >= endIndex) {
                    console.warn(
                        `Failed to find closing '}' character for property '${name}' (${filePath}).`
                    );
                    continue;
                }
                overloadEndIndex = definitionString.indexOf(
                    ";",
                    interfaceEndIndex
                );
                if (overloadEndIndex === -1 || overloadEndIndex >= endIndex) {
    ...
    if (parentName && currentDefinition) {
        ...
        for (
            ...
            currentDefinition.constants.push(
                `${constantMatch[1]}: ${constantMatch[2]}`
            );
        }
    }
...
function curryDefinition(
    ...
    let overloads = _.uniqWith(
        ...
        (a, b) =>
            ...
            _.isEqual(
                getUsedTypeParams(a.params, a.typeParams),
                getUsedTypeParams(b.params, b.typeParams)
            )
    );
    ...
    for (const overload of overloads) {
        for (let i = 0; i < overload.typeParams.length; ++i) {
            ...
            if (
                overload.params.every(p => !search.test(p)) &&
                !search.test(overload.returnType)
            ) {
        ...
        if (spreadIndex === arity - 1) {
            for (let i = 0; i < overloads.length; ++i) {
                ...
                if (
                    ...
                    // case 1
                    overload.params[spreadIndex] = overload.params[
                        spreadIndex
                    ].replace("...", "");
                } else if (
                    ...
                    const parts = overload.params[spreadIndex]
                        .split(":")
                        .map(_.trim);
                    ...
                    const spreadParamTypes = overload.params
                        .slice(spreadIndex)
                        .map(getParamType);
                    overload.params = overload.params.slice(0, spreadIndex);
                    overload.params.push(
                        `${paramName}: [${spreadParamTypes.join(", ")}]`
                    );
                } else {
            ...
            overloads = [
                {
                    ...
                    params: [
                        ...
                        ...overload.params.slice(
                            overload.params.length - (arity - spreadIndex - 1)
                        )
                    ],
                    returnType: "any"
                }
    ...
    let filteredOverloads = overloads.filter(
        o =>
            ...
            o.params
                .slice(arity)
                .every(p => p.includes("?") && !p.startsWith("iteratee")) &&
    ...
    if (filteredOverloads.length === 0)
        filteredOverloads = overloads.filter(
            o =>
                ...
                o.params
                    .slice(arity)
                    .every(p => p.includes("?") || p.startsWith("...")) &&
    ...
    if (filteredOverloads.length === 0)
        filteredOverloads = overloads.filter(
            o =>
                o.params.length > 0 &&
                o.params.length <= arity + 1 &&
                o.params[o.params.length - 1].startsWith("...")
        );
    if (filteredOverloads.length === 0)
        console.warn(
            `No matching overloads found for ${functionName} with arity ${arity}`
        );
    ...
    for (const restOverload of restOverloads) {
        restOverload.params[
            restOverload.params.length - 1
        ] = restOverload.params[restOverload.params.length - 1]
            .substring(3)
            .replace(/\[\]$/, "")
            .replace(/: Array<(.+)>$/, ": $1");
        if (restOverload.params.length < arity) {
            const paramToCopy =
                restOverload.params[restOverload.params.length - 1];
            const copiedParams = _.range(
                2,
                arity - restOverload.params.length + 2
            ).map(i => paramToCopy.replace(/(^.+?):/, `$1${i}:`));
    ...
    const interfaces = _.flatMap(filteredOverloads, (o, i) => {
        const reargParams = o.params.map(
            (p, i) => o.params[paramOrder.indexOf(i)]
        );
    ...
    const interfacesToMerge = interfaces.filter(
        i => i.name === mainInterface.name
    );
    mergeInterfaces(interfacesToMerge);
    _.remove(
        interfaces,
        i => i !== mainInterface && interfacesToMerge.includes(i)
    );
    ...
    for (const overload of mainInterface.overloads) {
        const others = mainInterface.overloads.filter(
            o2 =>
                ...
                _.isEqual(
                    overload.params.map(getParamType),
                    o2.params.map(getParamType)
                )
        );
        const returnInterface = interfaces.find(i =>
            new RegExp(`\\b${i.name}\\b`).test(overload.returnType)
        );
        for (const otherOverload of others) {
            for (let i = 0; i < overload.params.length; ++i) {
                const paramNames = _.uniq(
                    [overload]
                        .concat(others)
                        .map(o => getParamName(o.params[i]))
                );
            ...
            if (otherOverload.returnType === overload.returnType) {
                ...
                if (returnInterface && otherReturnInterface) {
                    _.remove(otherReturnInterface.overloads, o =>
                        new RegExp(`\\b${otherReturnInterface.name}\\b`).test(
                            o.returnType
                        )
                    );
                    ...
                    for (const overload of _.flatMap(
                        interfaces,
                        i => i.overloads
                    ))
                        overload.returnType = overload.returnType.replace(
...
function capCallback(parameter: string, functionName: string): string {
    ...
    if (
        ...
        /(?:iteratee|predicate|callback): *\((?:[^,)]+,[^)]+| *\.\.\..+)\) ?=>/.test(
            parameter
        )
    )
    ...
    return parameter
        ...
        .replace(
            /(iteratee|predicate|callback): *ObjectIteratee<(.+)>$/g,
            "$1: ValueIteratee<$2[keyof $2]>"
        )
        .replace(
        ...
        .replace(
            /(iteratee|predicate|callback): *StringIterator<(.+)>$/g,
            "$1: (value: string) => $2"
        )
        .replace(
            /(iteratee|predicate|callback): *ObjectIterator<(.+?), *(.+?)>$/g,
            "$1: (value: $2[keyof $2]) => $3"
        )
        .replace(
    ...
    for (const interfaceDef of interfaces) {
        for (const overload of interfaceDef.overloads) {
            const objectTypeParam = overload.typeParams.find(
                tp =>
                    tp.name === "T" && (tp.extends === "object" || !tp.extends)
            );
            if (
                objectTypeParam &&
                ...
                overload.params = overload.params.map(p =>
                    p
                        .replace(/\bArray<T\[keyof T]>/g, " T[]")
                        .replace(/\bT\[keyof T]/g, "T")
                );
                const fixInterfaceName = overload.returnType.split("<")[0];
                const fixInterface = interfaces.find(
                    i => i.name === fixInterfaceName
                );
                if (fixInterface) {
                    const fixTypeParam = fixInterface.typeParams.find(
                        tp =>
                            tp.name === objectTypeParam.name &&
                            (tp.extends === "object" || !tp.extends)
                    );
                    if (fixTypeParam) {
                        ...
                        for (const fixOverload of fixInterface.overloads) {
                            ...
                            if (matches.length >= 2) {
                                let i = 1;
                                const newTypeParam: TypeParam = {
                                    name: "T" + i,
                                    extends: replacement
                                };
                                while (
                                    fixInterface.typeParams.some(
                                        t => t.name === newTypeParam.name
                                    ) ||
                                    fixOverload.typeParams.some(
                                        t => t.name === newTypeParam.name
                                    )
                                )
                            ...
                            if (
                                !new RegExp(
                                    `\\b${baseName}[0-9]+x[0-9]+\\b`
                                ).test(fixOverload.returnType)
                            ) {
...
function curryParams(
    ...
    const interfaceTypeParams = getInterfaceTypeParams(
        sourceOverload,
        interfaceIndex
    );

    const prevParams = _.without(sourceOverload.params, ...params);
    const prevTypeParams = getUsedTypeParams(
        prevParams,
        sourceOverload.typeParams
    );
    ...
    for (let i = 1; i <= lastOverload; ++i) {
        ...
        const currentParams = params.map((p, j) =>
            i & (1 << j) ? p : `${getParamName(p)}: __`
        );
        while (
            currentParams.length > 0 &&
            _.last(currentParams)!.endsWith("__")
        )
            currentParams.pop(); // There's no point in passing a placeholder as the last parameter, so don't allow it.

        const usedTypeParams =
            i < lastOverload
                ? getUsedTypeParams(currentParams, typeParams)
                : typeParams;
        const combinedIndex = combineIndexes(interfaceIndex, i);
        const passTypeParams = getInterfaceTypeParams(
            sourceOverload,
            combinedIndex
        );
...
function getUsedTypeParams(params: string[], typeParams: TypeParam[]) {
    const usedTypeParams = typeParams.filter(tp =>
        params.some(p => new RegExp(`\\b${tp.name}\\b`).test(p))
    );
        ...
        ...typeParams.filter(
            tp =>
                ...
                usedTypeParams.some(
                    tp2 =>
                        !!tp2.extends &&
                        new RegExp(`\\b${tp.name}\\b`).test(tp2.extends)
                )
        )
    );
...
function getInterfaceTypeParams(overload: Overload, interfaceIndex: number) {
    ...
    const interfaceTypeParams = passedTypeParams.filter(tp =>
        currentParams
            .concat(overload.returnType)
            .some(p => new RegExp(`\\b${tp.name}\\b`).test(p))
    );
        ...
        ...overload.typeParams.filter(
            tp =>
                ...
                interfaceTypeParams.some(
                    tp2 =>
                        !!tp2.extends &&
                        new RegExp(`^keyof ${tp.name}\\b`).test(tp2.extends)
                )
        )
    );
    ...
    for (const overload of newOverloads) {
        ...
        const others = newOverloads.filter(
            o2 =>
                ...
                overload.params.every(
                    (p, i) => isPlaceholder(p) === isPlaceholder(o2.params[i])
                ) &&
                overload.params.filter(
                    (p, i) => getParamType(p) !== getParamType(o2.params[i])
                ).length <= 1 &&
        ...
        for (let i = 0; i < overload.params.length; ++i) {
            const paramNames = _.uniq(
                similarOverloads.map(o => getParamName(o.params[i]))
            );
            const newParamName =
                paramNames.length > 1
                    ? `${paramNames[0]}Or${paramNames
                          .slice(1)
                          .map(_.upperFirst)
                          .join("Or")}`
                    : paramNames[0];
            ...
            const newParamType = _(similarOverloads)
                ...
                .uniq()
                .sortBy(type =>
                    type === "undefined" ? 2 : type === "null" ? 1 : 0
                )
                .join(" | ");
...
function getInterfaceName(
    ...
    if (index > 0)
        interfaceName += `${overloadId}x${index}${typeParamsToString(
            typeParams,
            false
        )}`;
...
function interfaceToString(interfaceDef: Interface): string {
    if (_.isEmpty(interfaceDef.overloads)) {
        ...
        interfaceDef.overloads.length === 1 &&
        _.isEmpty(interfaceDef.constants)
    ) {
        ...
        overloadString = jsdoc
            ? lineBreak + tab(jsdoc + lineBreak + overloadString, 1)
            : " " + overloadString;
        return `type ${interfaceDef.name}${typeParamsToString(
            interfaceDef.typeParams
        )} =${overloadString}`;
    } else {
        const overloadStrings =
            interfaceDef.overloads
                .map(o => lineBreak + tab(overloadToString(o), 1))
                .join("") +
            interfaceDef.constants
                .map(c => `${lineBreak}${tab(c, 1)};`)
                .join("");
...
function overloadToString(overload: Overload, arrowSyntax = false): string {
    ...
    return `${jsdoc}${typeParamsToString(
        overload.typeParams
    )}(${joinedParams})${arrowSyntax ? " =>" : ":"} ${overload.returnType};`;
}
