...
var conn: mongoose.Connection = mongoose.createConnection(
...
db.openSet(
    "mongodb://user:pwd@localhost:27020/testing,mongodb://example.com:27020,mongodb://localhost:27019"
);
db.openSet(
...
var schema: mongoose.Schema = new Schema(
    { name: String },
    { collection: "actor" }
);
...
Model.create(
    { type: "jelly bean" },
    { type: "snickers" },
    (err: any, res1: IActor, res2: IActor) => {}
);
...
Model.find({ name: /john/i }, null, { skip: 10 }).exec(
    (err: any, docs: IActor[]) => {}
);
...
Model.findByIdAndRemove("id1", { select: "name" }).exec(
    (err: any, res: IActor) => {}
);
...
Model.findOne({ type: "iphone" }, "name", { lean: true }).exec(
    (err: any, res: IActor) => {}
);
...
Model.findOneAndRemove({ type: "iphone" }, { select: "name" }).exec(
    (err: any, res: IActor) => {}
);
...
Model.populate(
    { name: "Test A" },
    { path: "weapon", model: "Weapon" },
    (err: any, user: IActor) => {}
);
Model.populate(
    [{ name: "User hoge" }, { name: "User fuga" }],
    { path: "weapon" },
    (err: any, users: IActor[]) => {}
);
...
Model.update(
    { age: { $gt: 18 } },
    { oldEnough: true },
    (err: any, numberAffected: number, raw: any) => {}
);
Model.update(
    { name: "Tobi" },
    { ferret: true },
...
query3
    .find({ name: "Los Pollos Hermanos" })
    .find((err: any, res: IActor[]) => {});
query3
    .where("loc")
    .within()
    .geometry({
...
var id2: mongoose.Types.ObjectId = mongoose.Types.ObjectId.createFromHexString(
    "foo"
);
var isValid: boolean = mongoose.Types.ObjectId.isValid(
    "570d350b67b1ae0600e8bee8"
);
