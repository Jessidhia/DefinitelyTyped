 ...
 */
var conn1: mongoose.Connection = mongoose.createConnection(
 ...
 */
var validatorError: mongoose.Error.ValidatorError = new mongoose.Error.ValidatorError(
    { foo: "bar" }
);
...
var validationError: mongoose.Error.ValidationError = new mongoose.Error.ValidationError(
    doc
);
 ...
 */
var parallelSaveError: mongoose.Error.ParallelSaveError = new mongoose.Error.ParallelSaveError(
    doc
);
/* inherited properties */
 ...
 */
var overwriteModelError: mongoose.Error.OverwriteModelError = new mongoose.Error.OverwriteModelError(
    "foo"
);
/* inherited properties */
 ...
 */
var missingSchemaError: mongoose.Error.MissingSchemaError = new mongoose.Error.MissingSchemaError(
    "foo"
);
/* inherited properties */
 ...
 * https://mongoosejs.com/docs/api.html#mongooseerror_MongooseError.MissingSchemaError
 */
var missingSchemaError: mongoose.Error.DivergentArrayError = new mongoose.Error.DivergentArrayError(
    ["foo", "bar"]
);
/* inherited properties */
missingSchemaError.name;
...
interface PreHookTestModelInterface<T extends mongoose.Document>
...
preHookTestSchemaArr.push(
    schema.pre<PreHookTestModelInterface<PreHookTestDocumentInterface>>(
        "insertMany",
        function(next, docs) {
            const isSpecificType: PreHookTestModelInterface<
                PreHookTestDocumentInterface
            > = this;
            return Promise.resolve("");
        },
        err => {}
...
new mongoose.Schema({
    date: { type: Date, index: { unique: true, expires: "1d" } }
});
...
new mongoose.Schema({
    state: { type: String, enum: ["opening", "open", "closing", "closed"] }
});
new mongoose.Schema({
    state: {
        type: String,
...
new mongoose.Schema({}).plugin<PluginOption>(AwesomeLoggerPlugin, {
    modelName: "Executive",
    timestamp: "yyyy/MM/dd"
});

mongoose.plugin<PluginOption>(AwesomeLoggerPlugin, {
    modelName: "Executive",
    timestamp: "yyyy/MM/dd"
});
 ...
 */
var mongooseArray: mongoose.Types.Array<string> = new mongoose.Types.Array<
    string
>();
...
var newSub: MySubEntity1 = newEnt.sub.create({
    property1: "example",
    property2: "example"
});
 ...
 */
var decimal128: mongoose.Types.Decimal128 = mongoose.Types.Decimal128.fromString(
    "123.45678901234567"
);
 ...
 */
var objectId: mongoose.Types.ObjectId = mongoose.Types.ObjectId.createFromHexString(
    "0x1234"
);
...
var schemastring: mongoose.Schema.Types.String = new mongoose.Schema.Types.String(
    "hello"
);
 ...
 */
var schemanumber: mongoose.Schema.Types.Number = new mongoose.Schema.Types.Number(
    "num",
    {}
);
 ...
 */
var schemadate: mongoose.Schema.Types.Date = new mongoose.Schema.Types.Date(
    "99"
);
 ...
 */
var schemabuffer: mongoose.Schema.Types.Buffer = new mongoose.Schema.Types.Buffer(
    "99"
);
 ...
 */
var schemaboolean: mongoose.Schema.Types.Boolean = new mongoose.Schema.Types.Boolean(
    "99"
);
 ...
 */
var schemaobjectid: mongoose.Schema.Types.ObjectId = new mongoose.Schema.Types.ObjectId(
    "99"
);
 ...
 */
var schemamixed: mongoose.Schema.Types.Mixed = new mongoose.Schema.Types.Mixed(
    "99"
);
/* static properties */
 ...
 */
var schemaembedded: mongoose.Schema.Types.Embedded = new mongoose.Schema.Types.Embedded(
    new mongoose.Schema(),
    "99"
);
/* inherited properties */
...
mongopromise
    .complete(new mongoose.SchemaType(""))
    .complete(new mongoose.SchemaType(""), new mongoose.SchemaType(""));
/* static properties */
...
MongoModel.findById(999, function(err, doc) {
    ...
    doc.save(handleSave)
        .then(cb)
        .catch(cb);
    doc.save({ validateBeforeSave: false }, handleSave)
        .then(cb)
        .catch(cb);
    doc.save({ safe: true }, handleSave)
        .then(cb)
        .catch(cb);
...
MongoModel.find({ name: /john/i }, null, { skip: 10 }).exec(function(
    err,
    docs
) {});
...
MongoModel.findOne({ type: "iphone" }, "name").exec(function(
    err,
    adventure
) {});
...
MongoModel.deleteOne({ _id: "999" })
    .exec()
    .then(res => console.log(res.ok));
MongoModel.deleteMany({ _id: "999" }).then(res =>
    console.log("Success?", !!res.ok, "deleted count", res.n)
);
...
export var Final: MyModel = <MyModel>(
    mongoose.connection.model<MyDocument>("Final", mySchema)
);
...
export var Final2: MyModel = mongoose.model<MyDocument, MyModel>(
    "Final2",
    mySchema
);
Final2.staticMethod();
...
const extended: mongoose.Model<extended> = base.discriminator<extended>(
    "extendedS",
    schema
);
...
const aggregatePrototypeGraphLookup: mongoose.Aggregate<
    any
> = MyModel.aggregate([]).graphLookup({});
const addFieldsAgg: mongoose.Aggregate<
    any
> = aggregatePrototypeGraphLookup.addFields({});
...
const queryProrotypeErrorSetUnset: mongoose.Query<any> = MyModel.findById({})
    .error(null)
    .error(new Error("foo"));
...
db.createCollection("customers")
    ...
    // is committed.
    .then((customer: mongoose.Document[]) =>
        Customer.findOne({ name: "Test" }).exec()
    )
    // This `findOne()` will return the doc, because passing the `session`
    // means this `findOne()` will run as part of the transaction.
    .then(() =>
        Customer.findOne({ name: "Test" })
            .session(session)
            .exec()
    )
...
let animal2QueryHelpers = {
    byName<Q extends mongoose.DocumentQuery<any, Animal2>>(
        this: Q,
        name: string
    ) {
...
var Animal2 = mongoose.model<
    Animal2,
    mongoose.Model<Animal2, typeof animal2QueryHelpers>
>("Animal", animal2Schema);
Animal2.find()
    .byName("fido")
