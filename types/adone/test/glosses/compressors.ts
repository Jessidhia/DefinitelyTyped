namespace compressorsTests {
    ...
    namespace gzTests {
        ...
        gz.compress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        gz.compress(Buffer.from("hello"), { chunkSize: 4096 }).then(
            (x: Buffer) => {}
        );
        gz.compress(Buffer.from("hello"), { finishFlush: gz.Z_FINISH }).then(
            (x: Buffer) => {}
        );
        gz.compress(Buffer.from("hello"), { flush: gz.Z_NO_FLUSH }).then(
            (x: Buffer) => {}
        );
        ...
        gz.compress(Buffer.from("hello"), {
            level: gz.Z_BEST_COMPRESSION
        }).then((x: Buffer) => {});
        gz.compress(Buffer.from("hello"), {
            memLevel: gz.Z_BEST_COMPRESSION
        }).then((x: Buffer) => {});
        gz.compress(Buffer.from("hello"), { strategy: gz.Z_HUFFMAN_ONLY }).then(
            (x: Buffer) => {}
        );
        gz.compress(Buffer.from("hello"), { windowBits: 10 }).then(
            (x: Buffer) => {}
        );
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.Gzip;
            } = gz.compressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = gz.compressSync(Buffer.from("hello"), {
                level: gz.Z_BEST_COMPRESSION
        ...
        gz.decompress(Buffer.from("hello"), { chunkSize: 4096 }).then(
            (x: Buffer) => {}
        );
        gz.decompress(Buffer.from("hello"), { finishFlush: gz.Z_FINISH }).then(
            (x: Buffer) => {}
        );
        gz.decompress(Buffer.from("hello"), { flush: gz.Z_NO_FLUSH }).then(
            (x: Buffer) => {}
        );
        gz.decompress(Buffer.from("hello"), { info: true }).then(
            (x: { buffer: Buffer; engine: nodestd.zlib.Gunzip }) => {}
        );
        gz.decompress(Buffer.from("hello"), { windowBits: 9 }).then(
            (x: Buffer) => {}
        );
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.Gunzip;
            } = gz.decompressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = gz.decompressSync(Buffer.from("hello"), {
                windowBits: 9
    ...
    namespace deflateTests {
        ...
        deflate.compress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { chunkSize: 4096 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { dictionary: Buffer.from("123") })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { finishFlush: 0 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { flush: 0 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.Deflate }) => {});
        deflate
            .compress(Buffer.from("hello"), { level: 0 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { memLevel: 0 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { strategy: 0 })
            .then((x: Buffer) => {});
        deflate
            .compress(Buffer.from("hello"), { windowBits: 0 })
            .then((x: Buffer) => {});
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.Deflate;
            } = deflate.compressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = deflate.compressSync(Buffer.from("hello"), {
                level: 0
        ...
        deflate.rawCompress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { chunkSize: 4096 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), {
                dictionary: Buffer.from("123")
            })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { finishFlush: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { flush: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { info: true })
            .then(
                (x: { buffer: Buffer; engine: nodestd.zlib.DeflateRaw }) => {}
            );
        deflate
            .rawCompress(Buffer.from("hello"), { level: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { memLevel: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { strategy: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawCompress(Buffer.from("hello"), { windowBits: 0 })
            .then((x: Buffer) => {});
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.DeflateRaw;
            } = deflate.rawCompressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = deflate.rawCompressSync(Buffer.from("hello"), {
                level: 0
        ...
        deflate.decompress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        deflate
            .decompress(Buffer.from("hello"), { chunkSize: 4096 })
            .then((x: Buffer) => {});
        deflate
            ...
            .then((x: Buffer) => {});
        deflate
            .decompress(Buffer.from("hello"), { finishFlush: 0 })
            .then((x: Buffer) => {});
        deflate
            .decompress(Buffer.from("hello"), { flush: 0 })
            .then((x: Buffer) => {});
        deflate
            .decompress(Buffer.from("hello"), { info: true })
            .then((x: { buffer: Buffer; engine: nodestd.zlib.Inflate }) => {});
        deflate
            .decompress(Buffer.from("hello"), { windowBits: 0 })
            .then((x: Buffer) => {});
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.Inflate;
            } = deflate.decompressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = deflate.decompressSync(Buffer.from("hello"), {
                windowBits: 0
        ...
        deflate.rawDecompress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        deflate
            .rawDecompress(Buffer.from("hello"), { chunkSize: 4096 })
            .then((x: Buffer) => {});
        deflate
            .rawDecompress(Buffer.from("hello"), {
                dictionary: Buffer.from("123")
            })
            .then((x: Buffer) => {});
        deflate
            .rawDecompress(Buffer.from("hello"), { finishFlush: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawDecompress(Buffer.from("hello"), { flush: 0 })
            .then((x: Buffer) => {});
        deflate
            .rawDecompress(Buffer.from("hello"), { info: true })
            .then(
                (x: { buffer: Buffer; engine: nodestd.zlib.InflateRaw }) => {}
            );
        deflate
            .rawDecompress(Buffer.from("hello"), { windowBits: 0 })
            .then((x: Buffer) => {});
        ...
        {
            const a: Buffer = deflate.rawDecompressSync(
                Buffer.from("hello"),
                {}
            );
        }
        ...
        }
        {
            const a: {
                buffer: Buffer;
                engine: nodestd.zlib.Inflate;
            } = deflate.rawDecompressSync(Buffer.from("hello"), { info: true });
        }
        {
            const a: Buffer = deflate.rawDecompressSync(Buffer.from("hello"), {
                windowBits: 0
    ...
    namespace brotliTests {
        ...
        brotli
            ...
            .then((x: Buffer) => {});
        brotli
            .compress(Buffer.from("hello"), { lgblock: 0 })
            .then((x: Buffer) => {});
        brotli
            .compress(Buffer.from("hello"), { lgwin: 0 })
            .then((x: Buffer) => {});
        brotli
            .compress(Buffer.from("hello"), { mode: 0 })
            .then((x: Buffer) => {});
        brotli
            .compress(Buffer.from("hello"), { quality: 0 })
            .then((x: Buffer) => {});
        brotli
            .compress(Buffer.from("hello"), { size_hint: 0 })
            .then((x: Buffer) => {});
    ...
    namespace lzmaTests {
        ...
        {
            const a: number = lzma.rawEncoderMemusage([
                { id: "LZMA_FILTER_DELTA", dist: 1 }
            ]);
        }
        {
            const a: number = lzma.rawDecoderMemusage([
                { id: "LZMA_FILTER_DELTA", dist: 1 }
            ]);
        }
        {
        ...
        {
            const a: Stream = lzma.createStream(
                "easyEncoder",
                lzma.PRESET_DEFAULT
            );
        }
        ...
        lzma.compress(Buffer.from("hello"), lzma.PRESET_DEFAULT).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        lzma.compress(Buffer.from("hello"), { blockSize: 4096 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { bufsize: 0 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { check: 0 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), {
            filters: [{ id: "LZMA_FILTER_ARMTHUMB" }]
        }).then((x: Buffer) => {});
        lzma.compress(Buffer.from("hello"), { flags: 0 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { memlimit: 0 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { preset: 0 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { synchronous: false }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { threads: 10 }).then(
            (x: Buffer) => {}
        );
        lzma.compress(Buffer.from("hello"), { timeout: 10 }).then(
            (x: Buffer) => {}
        );
        ...
        lzma.decompress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        lzma.decompress(Buffer.from("hello"), { memlimit: 100 }).then(
            (x: Buffer) => {}
        );
    ...
    namespace xzTests {
        ...
        {
            const a: number = xz.rawEncoderMemusage([
                { id: "LZMA_FILTER_DELTA", dist: 1 }
            ]);
        }
        {
            const a: number = xz.rawDecoderMemusage([
                { id: "LZMA_FILTER_DELTA", dist: 1 }
            ]);
        }
        {
        ...
        {
            const a: Stream = xz.createStream("easyEncoder", {
                check: xz.CHECK_CRC32
            });
        }
        {
            const a: Stream = xz.createStream("easyEncoder", {
                preset: xz.PRESET_EXTREME
            });
        }
        ...
        {
            const a: Stream = xz.createStream(
                "aloneEncoder",
                xz.PRESET_DEFAULT
            );
        }
        ...
        xz.singleStringCoding(xz.createStream("streamDecoder"), "hello").then(
            (x: Buffer) => {}
        );
        ...
        xz.compress(Buffer.from("hello"), xz.PRESET_DEFAULT).then(
            (x: Buffer) => {}
        );
        xz.compress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        xz.compress(Buffer.from("hello"), { check: xz.CHECK_CRC32 }).then(
            (x: Buffer) => {}
        );
        xz.compress(Buffer.from("hello"), { preset: xz.PRESET_EXTREME }).then(
            (x: Buffer) => {}
        );
        ...
        {
            const a: Stream = xz.compressStream({
                preset: xz.PRESET_LEVEL_MASK
            });
        }

        xz.decompress(Buffer.from("hello")).then((x: Buffer) => {});
        xz.decompress(Buffer.from("hello"), {}).then((x: Buffer) => {});
        xz.decompress(Buffer.from("hello"), {
            flags: xz.LZMA_CONCATENATED
        }).then((x: Buffer) => {});
        xz.decompress(Buffer.from("hello"), { memlimit: 100 }).then(
            (x: Buffer) => {}
        );
