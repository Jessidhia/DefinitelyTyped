namespace adoneTests.collection.BinarySearchTree {
    ...
    type BinarySearchTree<K = any, V = any> = adone.collection.BinarySearchTree<
        K,
        V
    >;
    ...
    new BinarySearchTree({});
    new BinarySearchTree<string, number>({
        checkValueEquality: (a: number, b: number) => a === b
    });
    new BinarySearchTree<string, number>({
        compareKeys: (a: string, b: string) => a.length - b.length
    });
    new BinarySearchTree<string, number>({
        parent: new BinarySearchTree<string, number>()
    });
    new BinarySearchTree<string, number>({ unique: true });
    new BinarySearchTree<string, number>({ value: 123 });
    {
        const a: BinarySearchTree<string, number> = new BinarySearchTree<
            string,
            number
        >().getMaxKeyDescendant();
    }
    ...
    new BinarySearchTree<string, number>().checkAllNodesFullfillCondition(
        (key: string, value: number) => null
    );
    new BinarySearchTree<string, number>().checkIsBST();
    {
        const a: number = new BinarySearchTree<
            string,
            number
        >().getNumberOfKeys();
    }
    ...
    {
        const a: number[] = new BinarySearchTree<string, number>().search(
            "asd"
        );
    }
    {
    ...
    new BinarySearchTree<string, number>().executeOnEveryNode(
        (tree: BinarySearchTree<string, number>) => null
    );
