namespace adoneTests.collection.LRU {
    ...
    {
        const a: (v: number, k: string) => number = new LRU<string, number>()
            .lengthCalculator;
    }
    {
    ...
    {
        const a: Array<
            adone.collection.I.LRU.SerializedEntry<string, number>
        > = new LRU<string, number>().dump();
        const [b] = a;
        {
            const c: number = b.e;
        }
    ...
    {
        const a: adone.collection.LinkedList<
            adone.collection.I.LRU.Entry<string, number>
        > = new LRU<string, number>().dumpLru();
        const b = a.shift();
        if (b !== undefined) {
            {
    ...
    {
        const a: adone.collection.I.LRU.Entry<string, number> = new LRU<
            string,
            number
        >().pop();
    }
