namespace archivesTests {
    ...
    namespace tarTests {
        ...
        namespace rawUnpackStreamTest {
            ...
            new RawUnpackStream();
            new RawUnpackStream().on(
                "entry",
                (header: Header, stream: UnpackSourceStream, next) => {
                    {
        ...
        namespace unpackStreamTests {
            ...
            {
                const a: RawUnpackStream = unpackStream(".", {
                    unpack: new tar.RawUnpackStream()
                });
            }
            {
            ...
            {
                const a: RawUnpackStream = unpackStream(".", {
                    mapStream: (stream: UnpackSourceStream, header) => stream
                });
            }
            {
                const a: RawUnpackStream = unpackStream(".", {
    ...
    namespace zipTests {
        ...
        namespace packTests {
            ...
            {
                const a: ZipFile = file.addBuffer(
                    Buffer.from("buffer"),
                    "file"
                );
            }
            ...
            {
                const a: ZipFile = file.addReadStream(
        ...
        namespace unpackTests {
...

            {
                ...
                a.on("entry", async (entry: Entry<string>) => {
                    ...
                    {
                        const a: Array<{ id: number; data: Buffer }> =
                            entry.extraFields;
                    }
                    {
                    ...
                    {
                        const stream: nodestd.stream.Readable = await a.openReadStream(
                            entry
                        );
                    }
                    {
            ...
            {
                const a: ZipFile<Buffer> = unpack.open("hello", {
                    decodeStrings: false
                });
                {
            ...
            {
                const a: ZipFile<string> = unpack.fromBuffer(
                    Buffer.from("123")
                );
            }
            {
            ...
            {
                const a: ZipFile<Buffer> = unpack.fromBuffer(
                    Buffer.from("123"),
                    { decodeStrings: false }
                );
            }
            ...
            {
                const a: ZipFile<Buffer> = unpack.fromFd(0, {
                    decodeStrings: false
                });
            }
            {
            ...
            {
                const a: ZipFile<string> = unpack.fromRandomAccessReader(
                    new fs.RandomAccessBufferReader(Buffer.from("asd")),
                    1
                );
            }
            {
            ...
            {
                const a: ZipFile<Buffer> = unpack.fromRandomAccessReader(
                    new fs.RandomAccessBufferReader(Buffer.from("asd")),
                    1,
                    { decodeStrings: false }
                );
