namespace promiseTests {
    ...
    namespace nodeify {
        promise
            .nodeify(Promise.resolve(2), (err: any, value: number) => value)
            .then((x: number) => x);
    ...
    namespace callbackifyTests {
        ...
        callbackify(async () => {})((err: any, a: undefined) => {}).then(
            (x: undefined) => {}
        );
        callbackify(async () => 42)((err: any, a: number) => {}).then(
            (x: number) => {}
        );
        callbackify(async (a: number) => a)(
            123,
            (err: any, a: number) => {}
        ).then((x: number) => {});
        callbackify(async (a: number, b: string) => b)(
            123,
            "456",
            (err: any, a: string) => {}
        ).then((x: string) => {});
        ...
        callbackify(
            async (a: number, b: string, c: number, d: string, e: number) => e
        )(123, "456", 123, "456", 123, (err: any, a: number) => {}).then(
            (x: number) => {}
        );
    }
