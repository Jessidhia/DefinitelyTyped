namespace dataTests {
    ...
    namespace mpakTests {
        ...
        {
            const a: adone.collection.ByteArray = new mpak.Encoder([]).encode(
                "hello"
            );
        }
        {
        ...
        new mpak.Decoder([]).tryDecode(
            adone.collection.ByteArray.wrap("hello")
        );
    ...
    namespace json5Tests {
        ...
        json5.decode(
            Buffer.from("hello"),
            (holder: object, key: string, value: any) => null
        );
    }
    ...
    namespace base64Tests {
        ...
        {
            const a: { value: number; index: number } = base64.decodeVLQ(
                "H",
                1,
                true
            );
    ...
    namespace yamlTests {
        ...
        namespace loaderTests {
            ...
            loader.safeLoadAll("hello", undefined, {
                onWarning: warn => null
            })[0];
            loader.safeLoadAll("hello", undefined, {
        ...
        namespace schemaTests {
            ...
            {
                const a: adone.data.yaml.schema.Schema = new schema.Schema()
                    .include[0];
            }
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
                    .implicit[0];
            }
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
            ...
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
                    .compiledTypeMap.scalar.hello;
            }
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
                    .compiledTypeMap.sequence.hello;
            }
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
                    .compiledTypeMap.mapping.hello;
            }
            {
                const a: adone.data.yaml.type.Type = new schema.Schema()
                    .compiledTypeMap.fallback.hello;
            }

            {
                const a: adone.data.yaml.schema.Schema = schema.create(
                    schema.CORE,
                    yaml.type.Binary
                );
            }
        ...
        {
            const a: adone.data.yaml.Mark = new yaml.Exception("message", mark)
                .mark;
        }
    ...
    namespace bsonTests {
        ...
        {
            const a: string = new bson.DBRef("aa", new bson.ObjectId())
                ._bsontype;
        }
        new bson.DBRef("aa", new bson.ObjectId(), "hh");
        {
            const a: string = new bson.DBRef("aa", new bson.ObjectId()).toJSON()
                .$ref;
        }
        {
            const a: adone.data.bson.ObjectId = new bson.DBRef(
                "aa",
                new bson.ObjectId()
            ).toJSON().$id;
        }
        ...
        {
            const a: string = new bson.Decimal128(
        ...
        }
        {
            const a: string = new bson.Decimal128(
                Buffer.from("0123456789abcdef")
            ).toJSON().$numberDecimal;
        }
        {
            const a: adone.data.bson.Decimal128 = bson.Decimal128.fromString(
                "123"
            );
        }
        ...
        {
            const a: adone.data.bson.ObjectId = bson.ObjectId.createFromTime(
                100500
            );
        }
        {
            const a: adone.data.bson.ObjectId = bson.ObjectId.createFromHexString(
                "deadbeef"
            );
        }
        {
        ...
        {
            const a: adone.data.bson.Timestamp =
                bson.Timestamp.MAX_UNSIGNED_VALUE;
        }
        {
        ...
        {
            const a: Buffer = new bson.BSON().serialize(
                { a: 1 },
                { checkKeys: true }
            );
        }
        ...
        {
            const a: number = new bson.BSON().serializeWithBufferAndIndex(
                { a: 1 },
                Buffer.alloc(10)
            );
        }
        ...
        {
            const a: number = new bson.BSON().calculateObjectSize(
                { a: 1 },
        ...
        new bson.BSON().deserialize(Buffer.from("aahaha"), {}).a;
        new bson.BSON().deserialize(Buffer.from("aahaha"), {
            bsonRegExp: false
        }).a;
        new bson.BSON().deserialize(Buffer.from("aahaha"), {
        ...
        {
            const a: number = new bson.BSON().deserializeStream(
                Buffer.from("ahaha"),
                1,
                2,
                [],
