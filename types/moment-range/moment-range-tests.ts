...
const range: momentRange.DateRange = new momentRange.DateRange(
    new Date(2012, 0, 15),
    new Date(2012, 4, 23)
);
...
const range4: momentRange.DateRange = new momentRange.DateRange(
    "2015-01-17T09:50:04+00:00/2015-04-17T08:29:55+00:00"
);
...
const it2: Iterable<moment.Moment> = range.by("years", {
    exclusive: false,
    step: 2
});
...
const it5: Iterable<moment.Moment> = range.byRange(range, {
    exclusive: false,
    step: 2
});
...
const it7: Iterable<moment.Moment> = range.reverseBy("months", {
    exclusive: true
});
const it8: Iterable<moment.Moment> = range.reverseBy("years", {
    exclusive: false,
    step: 2
});

const it9: Iterable<moment.Moment> = range.reverseByRange(range);
const it10: Iterable<moment.Moment> = range.reverseByRange(range, {
    exclusive: true
});
const it11: Iterable<moment.Moment> = range.reverseByRange(range, {
    exclusive: false,
    step: 2
});
