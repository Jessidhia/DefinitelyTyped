...
class CustomRequestLogger implements cassandra.metrics.ClientMetrics {
    onAuthenticationError(
        e: Error | cassandra.errors.AuthenticationError
    ): void {
...
const client = new cassandra.Client({
    ...
    requestTracker: new cassandra.tracker.RequestLogger({
...
client
    .execute(query, ["guy"], { prepare: true, counter: false })
    .then(
...
const metadata = new cassandra.metadata.Metadata(
    cassandra.defaultOptions(),
    undefined
);
...
const tokenizer: cassandra.token.Tokenizer = {
    ...
    split: (
        start: cassandra.token.Token,
        end: cassandra.token.Token,
...
const range: cassandra.token.TokenRange = new cassandra.token.TokenRange(
    start,
    end,
    tokenizer
);

class MyLoadBalancingPolicy extends cassandra.policies.loadBalancing
    .LoadBalancingPolicy {
    getDistance() {
...
existingPolicy = new cassandra.policies.loadBalancing.DCAwareRoundRobinPolicy(
    "dc"
);
