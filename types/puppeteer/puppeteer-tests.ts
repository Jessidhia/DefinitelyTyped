...
(async () => {
    ...
    const buttonText = await (await button.getProperty(
        "textContent"
    )).jsonValue();
    ...
    console.log(
        ...
        navResponse.status(),
        navResponse.url(),
        navResponse.headers()["Content-Type"]
    );
...
async () => {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    const eh = (await page.$("tr.something")) as puppeteer.ElementHandle<
        HTMLTableRowElement
    >;
...
(async () => {
    ...
    const stringOrBuffer: string | Buffer = await page.screenshot(
        screenshotOptions
    );
...
(async () => {
    ...
    const nullableEl: puppeteer.ElementHandle | null = await page.waitFor(
        "selector",
        {
            hidden: true
        }
...
async () => {
    ...
    const s = await page.evaluate(() =>
        Promise.resolve(document.body.innerHTML)
    );
    console.log("body html has length", s.length);
};
