...
function test_datafactory() {
    ...
    const variable: Variable = dataFactory.variable
        ? dataFactory.variable("v1")
        : <any>{};
...
function test_stream() {
    ...
    const eventEmitter10: EventEmitter = store.removeMatches(
        term,
        term,
        term,
        term
    );
    const eventEmitter11: EventEmitter = store.removeMatches(
        term,
        term,
        term,
        /.*/
    ...
    const eventEmitter13: EventEmitter = store.deleteGraph(
        "http://example.org"
    );
}
