...
nsNames.forEach(ns =>
    checkExist(ansiStyles[ns], `ansiStyles.${ns} is not exist.`)
);

["color", "bgColor"].forEach(ns =>
    checkConverter(ns, ansiStyles[ns], colorFormats)
);
...
function checkCodePair(styleName: string, pair: any): void {
    codePair.forEach(p =>
        checkIsString(pair[p], `${styleName}.${p} is not a string.`)
    );
}
...
function checkConverter(nsName: string, namespace: any, formats: string[]) {
    formats.forEach(f =>
        codeTypes.forEach(t =>
            checkIsFunction(
                namespace[t][f],
                `ansiStyles.${nsName}.${t}.${f} is not a function.`
            )
        )
    );
