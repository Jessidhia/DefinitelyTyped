...
// https://msdn.microsoft.com/en-us/library/jj249434.aspx
{
    withConnection("Northwind", conn => {
        withEmployees(rs => {
            ...
            WScript.Echo(
                `There are ${pageCount} pages, each containing ${
                    rs.PageSize
                } or fewer records`.trim()
            );
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                for (let iRecord = 1; iRecord <= rs.PageSize; iRecord++) {
                    ...
                    const hireDate = new Date(rs("HireDate")
                        .Value as VarDate).toString();
    ...
    withConnection("Northwind", conn => {
        withEmployees(rs => {
            ...
            while (!rs.EOF) {
                ...
                const recordCount = `${rs.AbsolutePosition} of ${
                    rs.RecordCount
                }`;
