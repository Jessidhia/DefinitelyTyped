...
function test_dom_comment() {
    ...
    var comment3 = new CKEDITOR.dom.comment(
        "Example",
...
function test_dom_element() {
    ...
    var first2 = element.getFirst((node: CKEDITOR.dom.node) =>
    ...
    var last2: CKEDITOR.dom.node = element.getLast((node: CKEDITOR.dom.node) =>
        node.equals(document.body)
    );
    ...
    var element6: CKEDITOR.dom.element = CKEDITOR.dom.element.get(
        document.createElement("div")
    );
    var element3: CKEDITOR.dom.element = CKEDITOR.dom.element.get(element2);
    var markedElement: CKEDITOR.dom.element = CKEDITOR.dom.element.setMarker(
        {},
        element,
        "key",
        0
...
function test_dom_elementPath() {
    var path: CKEDITOR.dom.elementPath = new CKEDITOR.dom.elementPath(
        CKEDITOR.dom.element.get("div")
    );
    var path2: CKEDITOR.dom.elementPath = new CKEDITOR.dom.elementPath(
        CKEDITOR.dom.element.get("div"),
...
function test_dom_selection() {
    ...
    var selection: CKEDITOR.dom.selection = new CKEDITOR.dom.selection(
        new CKEDITOR.dom.document(document)
    );
    selection = new CKEDITOR.dom.selection(element);
    selection = new CKEDITOR.dom.selection(selection);

    var marks: CKEDITOR.dom.bookmark[] = selection.createBookmarks(
        "serialized"
    );
    var marks2: CKEDITOR.dom.bookmark2[] = selection.createBookmarks2({
        normalized: true
    });
...
function test_ajax() {
    ...
    req = CKEDITOR.ajax.load("https://ckeditor.com/ckeditor-4/", (data: any) =>
        console.log(data)
    );

    var xml: CKEDITOR.xml = CKEDITOR.ajax.loadXml(
...
function test_dialog() {
    var dialog: CKEDITOR.dialog = new CKEDITOR.dialog(
    ...
    var uiEl: CKEDITOR.ui.dialog.uiElement = dialog.getContentElement(
        "page1",
        "element1"
    );
    ...
    var isEnabled: boolean = CKEDITOR.dialog.isTabEnabled(
        editor,
        "dialog",
        "table"
    );
...
function test_fileTools_fileLoader() {
    ...
    var loader: CKEDITOR.fileTools.fileLoader = new CKEDITOR.fileTools.fileLoader(
        editor,
        "data"
    );
...
function test_fileTools_uploadRepository() {
    var editor: CKEDITOR.editor;
    var blob: Blob;
    var repo: CKEDITOR.fileTools.uploadRepository = new CKEDITOR.fileTools.uploadRepository(
        editor
    );
...
function test_filter() {
    ...
    var apply: boolean = filter.applyTo(
...
function test_htmlParser_comment() {
    ...
    var comment: CKEDITOR.htmlParser.comment = new CKEDITOR.htmlParser.comment(
        "hey"
    );
...
function test_htmlParser_cssStyle() {
    ...
    var style: CKEDITOR.htmlParser.cssStyle = new CKEDITOR.htmlParser.cssStyle(
        element
    );
...
function test_htmlParser_element() {
    var element: CKEDITOR.htmlParser.element = new CKEDITOR.htmlParser.element(
        "el",
        { id: "1" }
    );
...
function test_htmlParser_filter() {
    var filter: CKEDITOR.htmlParser.filter = new CKEDITOR.htmlParser.filter({
        elementNames: ["div"]
    });
...
function test_htmlParser_fragment() {
    ...
    var fragOrEl:
        | CKEDITOR.htmlParser.fragment
        | CKEDITOR.htmlParser.element = CKEDITOR.htmlParser.fragment.fromHtml(
        "html"
    );
...
function test_keystrokeHandler() {
    var handler: CKEDITOR.keystrokeHandler = new CKEDITOR.keystrokeHandler(
        CKEDITOR.instances[0]
    );
...
function test_resourceManager() {
    var manager: CKEDITOR.resourceManager = new CKEDITOR.resourceManager(
        "path",
        "name"
    );
...
function test_style() {
    var style: CKEDITOR.style = new CKEDITOR.style(
        { element: "div" },
        { key: "value " }
    );
...
function test_tools() {
    ...
    var numb: number = CKEDITOR.tools.getIndex<string>(
        ["string", "array"],
        (element: string) => false
    );
    bool = CKEDITOR.tools.getMouseButton(
        new CKEDITOR.dom.event(document.createEvent(null))
    );
