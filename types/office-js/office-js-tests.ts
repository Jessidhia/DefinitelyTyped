...
function test_word() {
    ...
    Word.run(function(context) {
        ...
        return context.sync().then(function() {
            console.log(
                "Created content control with id: " + myContentControl.id
            );
        });
    }).catch(function(error) {
        console.log("Error: " + JSON.stringify(error));
    ...
    Word.run(function(context) {
        ...
        var image = body.insertInlinePictureFromBase64(
            "",
    ...
    Word.run(context => {
        ...
        return (
            context
                ...
                .then(() => {
                    // Queue a command to insert the image into the document.
                    var insertedImage = imageRange.insertInlinePictureFromBase64(
                        "",
...
async function test_visio() {
    ...
    try {
        ...
        await Visio.run(session, async context => {
            const eventResult = context.document.onPageLoadComplete.add(
                async args => {
                    console.log(
                        Date.now() +
...
function test_OfficePromise() {
    ...
    let p2: Promise<any> = new OfficeExtension.Promise(resolve =>
        setTimeout(resolve, 1000)
    );
    let p3: Promise<any> = new Office.Promise(resolve =>
        setTimeout(resolve, 1000)
    );
    let p4: OfficeExtension.IPromise<any> = new OfficeExtension.Promise(
...
async function testResumeExistingObject() {
    ...
    await Excel.run(
        { delayForCellEdit: true, previousObjects: range },
        async context => {
            range.clear();
            await context.sync();
        }
