...
function StepSample(this: cucumber.StepDefinitions & cucumber.Hooks) {
    ...
    hook.Before(
        { timeout: 1000 },
        (scenario: HookScenario, callback: Callback) => {
            callback();
        }
    );
    ...
    hook.Around(
        (
            ...
            runScenario: (error: string | null, callback?: () => void) => void
        ) => {
    ...
    hook.registerHandler(
        "AfterFeatures",
        (event: any, callback: () => void) => {
            callback();
        }
    );

    step.Given(/^I am on the Cucumber.js GitHub repository$/, function(
        callback: Callback
    ) {
    ...
    step.When(
        ...
        (title: string, callback: Callback) => {
    ...
    step.Then(
        ...
        { timeout: 60 * 1000 },
        function(title: string, callback: Callback) {
            ...
            if (title === pageTitle) {
                ...
                callback(
                    new Error("Expected to be on page with title " + title)
                );
            }
        }
    );
...
function registerListener(): cucumber.EventListener {
    const listener = Object.assign(cucumber.Listener(), {
        handleBeforeScenarioEvent(
            scenario: cucumber.events.ScenarioPayload,
            callback: () => void
        ) {
