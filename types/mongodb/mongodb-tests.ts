...
MongoClient.connect(connectionString, options, function(
    ...
    collection.insertOne({ a: 2 }, function(
        err: mongodb.MongoError,
        docs: any
    ) {
        ...
        collection.countDocuments({ foo: 1 }, function(
            err: mongodb.MongoError,
            count: number
        ) {
            console.log(format("count = %s", count));
        ...
        collection.countDocuments({ foo: 1 }, { limit: 10 }, function(
            err: mongodb.MongoError,
            count: number
        ) {
            console.log(format("count = %s", count));
        ...
        collection
            .find({})
            .toArray(function(err: mongodb.MongoError, results: any) {
                console.dir(results);
                // Let's close the db
                client.close();
            });
        ...
        collection.createIndex(
            {},
            { partialFilterExpression: { rating: { $exists: 1 } } }
        );
    });
    ...
    // Collection .findM<T>() & .agggregate<T>() generic tests
    {
        ...
        type bag = typeof bag;
        let cursor: mongodb.Cursor<bag> = collection.find<bag>({
            color: "black"
        });
    ...
    }
    {
        ...
        let cursor: mongodb.AggregationCursor<payment> = collection.aggregate<
            payment
        >([{}]);
        ...
        collection.aggregate<payment>([{ $match: { bar: 1 } }], function(
            err: mongodb.MongoError,
            cursor: mongodb.AggregationCursor<payment>
        ) {
            cursor.limit(10);
        });

        collection.aggregate<payment>([], function(
            err: mongodb.MongoError,
            cursor: mongodb.AggregationCursor<payment>
        ) {
    ...
    // test for new typings
    {
        ...
        const res: mongodb.Cursor<TestCollection> = testCollection.find({
            _id: 123
        });
    }
});
