...
function test_ChartOptions() {
    ...
    $("#container").highcharts({
        ...
        subtitle: {
            text:
                "Split tooltips in Highcharts makes it easier to read overlapping line series"
        },
...
function test_AreaRange() {
    ...
    // datalabels example
    $.getJSON(
        "http://www.highcharts.com/samples/data/jsonp.php?filename=range.json&callback=?",
        data => {
            // Shorten the data
...
function test_ChartObject() {
    ...
    chart.addAxis({}, true, true, false);
    chart.addAxis({}, true, true, { duration: 50 });
    chart.addSeries({});
...
function test_ResponsiveOptions() {
    const responsiveOptions: Highcharts.ResponsiveOptions = {
        rules: [
            {
                chartOptions: {
                    chart: {
...
function test_WordCloud() {
    ...
    const series: Highcharts.WordCloudChartSeriesOptions = {
        ...
        tooltip: {
            headerFormat: null,
            pointFormatter() {
                return `<strong>${this.name}</strong>: Occurrence ${
                    this.weight
                }`;
            }
        }
