...
async () => {
    ...
    var job = agenda.create<{ userCount: number }>("printAnalyticsReport", {
        userCount: 100
    });
    await job.save();
...
const extendedAgenda: ExtendedAgenda = new ExtendedAgenda().mongo(
    new Db("some-database", new Server("host.name", 0))
);
