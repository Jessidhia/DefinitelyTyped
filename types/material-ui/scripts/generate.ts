...
const github = path =>
    new Promise((resolve, reject) => {
        get(
            {
                headers: {
                    "user-agent": "DefinitelyTyped/material-ui/generate"
                },
                host: "api.github.com",
                path
            },
...
const categories = () =>
    github(
        `/repos/mui-org/material-ui/contents/src/svg-icons?ref=master&access_token=${token}`
    );

const contents = path =>
    github(
        `/repos/mui-org/material-ui/contents/${path}?ref=master&access_token=${token}`
    );
...
async function main() {
    ...
    const contentsList = await Promise.all(
        Array.prototype.map.call(cats, cat =>
            contents(cat.path).then(cons =>
                Array.prototype.map.call(cons, con => {
    ...
    const { dts, test } = await Array.prototype.concat
        .apply([], contentsList)
        .map(content => {
            const { path } = content;
            ...
            content.className =
                toMixedCase(content.category) + toMixedCase(name);
            return content;
        })
        ...
        .reduce(
            (prev, content) => {
                const { dts, test } = prev;
                dts.individuals.push(`declare module 'material-ui/${
                    content.id
                }' {
    export import ${content.className} = __MaterialUI.SvgIcon;
    export default ${content.className};
}`);
                dts.summarizeds.push(
                    `    export import ${
                        content.className
                    } = __MaterialUI.SvgIcon; // require('material-ui/${
                        content.id
                    }');`
                );

                test.individuals.push(
                    `import _${content.className} from 'material-ui/${
                        content.id
                    }';`
                );
