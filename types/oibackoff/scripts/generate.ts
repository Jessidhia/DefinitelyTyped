...
function generateBackoffInstance() {
    ...
    for (
        let nCallbackParameters = 1;
        nCallbackParameters < 5;
        nCallbackParameters++
    ) {
...
function generateSingle(
    ...
    s += join(
        nCallbackParameters,
        i => `${callbackParameterName(i)}: ${callbackParameterTypeName(i)}`
    );
    s += ") => void) => void,\n\t\t";
                ...
                switch (i) {
                    ...
                    default:
                        return `${callbackParameterName(
                            i
                        )}: ${callbackParameterTypeName(i)} | undefined`;
                }
            });
            break;
    }
