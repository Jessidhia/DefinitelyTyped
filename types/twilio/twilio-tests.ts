...
resp.say("Welcome to Acme Customer Service!").gather(
    ...
    function() {
        this.say("Press 1 for customer service").say(
            "Press 2 for British customer service",
            { language: "en-gb" }
        );
    }
);
...
resp.say("Welcome to Acme Customer Service!").gather(
    ...
    function(node: twilio.Node) {
        ...
        node.say("Press 1 for customer service").say(
            "Press 2 for British customer service",
            { language: "en-gb" }
        );
    }
...
/// Utilities
twilio.validateRequest(token, str, "http://example.herokuapp.com", {
    query: "val"
});
twilio.validateExpressRequest(
    getMockExpressRequest(),
