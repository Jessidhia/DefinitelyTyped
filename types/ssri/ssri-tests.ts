...
const parseStringSingle: ssri.Hash = ssri.parse(integrityString, {
    single: true,
    strict: true
});
const parseStringWithOptions: ssri.IntegrityMap = ssri.parse(integrityString, {
    strict: true
});
...
const integrityConcatString: ssri.IntegrityMap = new ssri.Integrity().concat(
    integrityString
);
const integrityConcatHash: ssri.IntegrityMap = new ssri.Integrity().concat(
    hash
);
const integrityConcatIntegrity: ssri.IntegrityMap = new ssri.Integrity().concat(
    integrity
);
...
const integrityToStringWithOptions: string = new ssri.Integrity().toString({
    strict: false,
    sep: ","
});
...
const integrityMatchString: ssri.Hash | false = new ssri.Integrity().match(
    integrityString
);
const integrityMatchHash: ssri.Hash | false = new ssri.Integrity().match(hash);
const integrityMatchIntegrity: ssri.Hash | false = new ssri.Integrity().match(
    integrity
);
const integrityMatchStringWithOptions:
    | ssri.Hash
    | false = new ssri.Integrity().match(integrityString, {
    strict: true,
    pickAlgorithm: (a, b) => a
});

const integrityPickAlgorithm: string = new ssri.Integrity().pickAlgorithm();
const integrityPickAlgorithmWithOptions: string = new ssri.Integrity().pickAlgorithm(
    { pickAlgorithm: (a, b) => a }
);
...
const fromHexSingle: ssri.Hash = ssri.fromHex("75e69d6de79f", "sha1", {
    single: true,
    strict: false,
    options: ["sriOpt"]
});
...
const fromDataBuffer: ssri.IntegrityMap = ssri.fromData(
...
const fromStream: PromiseLike<ssri.IntegrityMap> = ssri.fromStream(
...
const checkDataString: ssri.Hash | false = ssri.checkData(
    "data",
    integrityString
);
...
const checkStreamString: PromiseLike<ssri.Hash> = ssri.checkStream(
    createReadStream("index.js"),
    integrityString
);
const checkStreamHash: PromiseLike<ssri.Hash> = ssri.checkStream(
    createReadStream("index.js"),
    hash
);
const checkStreamIntegrity: PromiseLike<ssri.Hash> = ssri.checkStream(
    createReadStream("index.js"),
    integrity
);
