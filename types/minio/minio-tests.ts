...
minio.listBuckets(
    (error: Error | null, bucketList: Minio.BucketItemFromList[]) => {
...
minio.getObject(
    "testBucket",
    "hello.jpg",
    (error: Error | null, objectStream: Stream) => {
...
minio.statObject(
    "testBucket",
    "hello.jpg",
    (error: Error | null, stat: Minio.BucketItemStat) => {
...
minio.presignedPostPolicy(
    policy,
    (error: Error | null, data: Minio.PostPolicyResult) => {
        console.log(error, data);
    }
);
minio.presignedPostPolicy(policy);

minio.getBucketNotification(
    "testBucket",
    (error: Error | null, config: Minio.NotificationConfig) => {
...
const arn: string = Minio.buildARN(
    "aws",
    "sns",
    "us-west-2",
    "408065449417",
...
minio.listenBucketNotification("testBucket", "pref_", "_suf", [
