...
function TestSerializable() {
    let serial:
        | string
        | webdriver.promise.IThenable<string> = serializable.serialize();
}
...
function TestWebElementPromise() {
    ...
    elementPromise
        .then((element: webdriver.WebElement) => "foo", (error: any) => {})
        .then((result: string) => {});
}
...
function TestPromiseModule() {
    ...
    let numbersPromise: webdriver.promise.Promise<
        number[]
...
function TestUntilModule() {
    ...
    let conditionB: webdriver.Condition<boolean> = new webdriver.Condition<
        boolean
    >("message", (driver: webdriver.WebDriver) => true);
    ...
    let conditionAlert: webdriver.Condition<
        webdriver.Alert
...
function TestPromiseClass() {
    ...
    const promiseOrVoid: webdriver.promise.Promise<
        string | void
    > = promise.then((a: string) => "cde", (e: any) => {});
    const promiseOrNumber: webdriver.promise.Promise<
        string | number
    > = promise.then((a: string) => "cde", (e: any) => 123);
}

function TestThenableClass() {
    // TODO: this doesn't test the Thenable class, it uses a Promise!
    let thenable: webdriver.promise.Promise<
        string
    ...
    // tslint:disable-next-line void-return (need `--strictNullChecks` to change `void` to `undefined`)
    const thenableOrVoid: webdriver.promise.Promise<
        string | void
    > = thenable.then((a: string) => "cde", (e: any) => {});
    const thenableOrNumber: webdriver.promise.Promise<
        string | number
    > = thenable.then((a: string) => "cde", (e: any) => 123);
}
