...
knex = Knex({
    ...
    pool: {
        min: 0,
        max: 7,
        afterCreate: (connection: any, callback: (...args: any[]) => void) =>
            callback(null, connection),
        beforeDestroy: (connection: any, callback: (...args: any[]) => void) =>
            callback(null, connection)
    }
});
...
knex.column("title", { by: "author" }, "year")
    .select()
    .from("books");
knex.column({ title: "title", by: "author", published: "year" })
    .select()
    .from("books");
...
knex.select("*")
    .from("users")
    .join("accounts", (join: Knex.JoinClause) => {
        if (this !== join) {
        ...
        join.on("accounts.id", "=", "users.account_id").orOn(
            "accounts.owner_id",
            "=",
            "users.id"
        );
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").onIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").andOnIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").orOnIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").onNotIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").andOnNotIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").orOnNotIn("contacts.id", [
            7,
            15,
            23,
            41
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").onBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").andOnBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").orOnBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").onNotBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").andOnNotBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .join("contacts", () => {
        this.on("users.id", "=", "contacts.id").orOnNotBetween("contacts.id", [
            5,
            30
        ]);
    });
...
knex.select("*")
    .from("users")
    .leftJoin("accounts", (join: Knex.JoinClause) => {
        join.on("accounts.id", "=", "users.account_id").orOn(
            "accounts.owner_id",
            "=",
            "users.id"
...
knex.select("*")
    .from("users")
    .rightJoin("accounts", (join: Knex.JoinClause) => {
        join.on("accounts.id", "=", "users.account_id").orOn(
            "accounts.owner_id",
            "=",
            "users.id"
...
knex.select("*")
    .from("users")
    .outerJoin("accounts", (join: Knex.JoinClause) => {
        join.on("accounts.id", "=", "users.account_id").orOn(
            "accounts.owner_id",
            "=",
            "users.id"
...
knex.select("*")
    .from("users")
    .fullOuterJoin("accounts", (join: Knex.JoinClause) => {
        join.on("accounts.id", "=", "users.account_id").orOn(
            "accounts.owner_id",
            "=",
            "users.id"
...
knex("books")
    .where("published_date", "<", 2000)
    .update({ status: "archived" }, ["id", "title"]);
...
knex.with(
    "new_books",
    knex.raw("select * from books where published_date >= 2016")
)
    .select("*")
    .from("new_books");

knex.with("new_books", "select * from books where published_date >= :year", {
    year: 2016
})
    .select("*")
    .from("new_books");
...
knex.withRaw(
    "recent_books",
    "select * from books where published_date >= :year",
    { year: 2013 }
)
    .select("*")
    .from("recent_books");
...
knex.withRaw(
    "recent_books",
    knex.raw("select * from books where published_date >= ?", [2013])
)
    .select("*")
    .from("recent_books");
...
knex.table("users")
    .first(knex.raw("round(sum(products)) as p"))
    .then(row => {
        console.log(row);
    });
...
knex.schema.raw("SET sql_mode='TRADITIONAL'").table("users", table => {
    table.dropColumn("name");
    table.string("first_name");
    table.string("last_name");
...
knex.schema.dropTableIfExists("A").createTable("A", table => {
    table
        .integer("C")
        .unsigned()
        .references("B.id")
