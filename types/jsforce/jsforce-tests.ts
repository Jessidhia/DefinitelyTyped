...
async function testSObject(connection: sf.Connection) {
    ...
    const dummySObject: SObject<DummyRecord> = connection.sobject<DummyRecord>(
        "Dummy"
    );
...
async function testExecuteAnonymous(conn: sf.Connection): Promise<void> {
    ...
    console.log(
        "ExecuteAnonymousResult exceptionStackTrace: " + res.exceptionStackTrace
    );
...
async function testChatter(conn: sf.Connection): Promise<void> {
    ...
    chatter.resource("/feed-elements").create(
        ...
        (err: Error | null, result: any) => {
            ...
            const feedMessageUrl = `/feed-elements/${
                result.id
    ...
    const resourceMe: sf.Resource<sf.RequestResult> = chatter.resource(
        "/users/me"
    );
    ...
    const feedResource: sf.Resource<sf.RequestResult> = chatter.resource(
        "/feed-elements"
    );
    ...
    const itemLikesUrl = `/feed-elements/${
        feedCreateRequest.id
    }/capabilities/chatter-likes/items`;
    const itemsLikeResource: sf.Resource<sf.RequestResult> = chatter.resource(
        itemLikesUrl
    );
    ...
    console.log(
        `itemsLikeCreateResult['likedItem']: ${itemsLikeCreateResult as any["likedItem"]}`
    );
}
...
async function testDescribe() {
    ...
    globalCached.sobjects.forEach(
        async (sobject: sf.DescribeGlobalSObjectResult) => {
            const object: sf.DescribeSObjectResult = await salesforceConnection.describe(
                sobject.name
            );
            const cachedObject: sf.DescribeSObjectResult = salesforceConnection.describe$(
                sobject.name
            );
