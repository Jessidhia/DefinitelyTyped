...
Debug: ${server.Debug}
Build and version: ${server.MajorBuild}.${server.MinorBuild}.${
        server.MajorVersion
    }.${server.MinorVersion}
...
Is device used: ${rule.UseDevice}`.trim()
    ...
    WScript.Echo(
        `There are ${
            outboundRoutingRules.Count
        } outbound routing rules on this server.`
    );
...
`.trim();
    ...
    WScript.Echo(
        `There are now ${
            outboundRoutingGroups.Count
        } routing groups on the server`
    );
...
// https://msdn.microsoft.com/en-us/library/windows/desktop/ms693393(v=vs.85).aspx
{
    ...
    WScript.Echo(
        `There are ${outgoingQueue.GetJobs().Count} faxes in the outgoing queue`
    );
...
// https://msdn.microsoft.com/en-us/library/windows/desktop/ms693479(v=vs.85).aspx
{
    ...
    collectionToArray(recipients).forEach((recipient, index) =>
        WScript.Echo(
            `Recipient number ${index}: ${recipient.Name}, ${
                recipient.FaxNumber
            }`
        )
    );
...
// https://msdn.microsoft.com/en-us/library/windows/desktop/aa964962(v=vs.85).aspx
{
    ...
    const messageIterator = server.Folders.IncomingArchive.GetMessages(
        prefetchCount
    );
...
Build and version: ${provider.MajorBuild}.${provider.MajorVersion}.${
                    provider.MinorBuild
                }.${provider.MinorVersion}
