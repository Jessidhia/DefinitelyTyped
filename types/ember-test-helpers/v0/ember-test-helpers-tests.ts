...
QUnit.test("it sets up this.owner", function(
    this: TestContext,
    assert: Assert
) {
    const { owner } = this;
...
QUnit.test('can pauseTest to be resumed "later"', async function(
    this: TestContext,
    assert: Assert
) {
...
QUnit.test("render exposes an `.element` property", async function(
    this: TestContext,
    assert: Assert
) {
