...
seneca.act("cmd:salestax,net:100,country:IE,category:reduced", function(
    err,
    result
) {
...
seneca.add<SalestaxPattern, SalestaxParams>({ cmd: "salestax" }, function(
    args,
    callback
) {
    var rate = 0.23;
...
seneca.act<SalesTaxRequest>({ cmd: "salestax", net: 100 }, function(
    err,
    result
) {
    console.log(result.total);
...
seneca.add<ConfigPattern, ConfigParams>({ cmd: "config" }, function(
    args,
    callback
) {
    var config: any = {
...
seneca.add<SalestaxPattern, SalestaxParams>({ cmd: "salestax" }, function(
    ...
    seneca.act<ConfigRequest>({ cmd: "config", prop: "rate" }, function(
        err,
        result
    ) {
        var rate = parseFloat(result.value);
